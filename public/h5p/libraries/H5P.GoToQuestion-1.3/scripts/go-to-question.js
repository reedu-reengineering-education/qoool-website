H5P.GoToQuestion = (function ($, EventDispatcher, UI) {
  ...
  function GoToQuestion(parameters) {
      ...
      var $button = $("<div/>", {
        ...
        on: {
          ...
          keypress: function (event) {
            if (
              event.which === KEY_CODE_ENTER ||
              event.which === KEY_CODE_SPACE
            ) {
              // Space bar pressed
              choose.call(this, choiceParams)
    ...
    var choose = function (choiceParams) {
      ...
      // Disable all buttons
      var $buttons = $choices
        .find("." + GoToQuestion.htmlClass + "-button")
      ...
      $button
        .addClass(GoToQuestion.htmlClass + "-chosen")
        .css("top", $button[0].offsetTop + borderWidth)
        ...
        setTimeout(function () {
          ...
          $button.css(
            ...
            $text[0].getBoundingClientRect().height -
              parseFloat(buttonStyle.paddingTop)
          )
        }, 0)
    ...
    var continueScreen = function (chosenText, continueMsg, goTo) {
      ...
      $continueButton
        .appendTo($wrapper)
        .on("click", createContinueHandler(goTo))
        .focus()
