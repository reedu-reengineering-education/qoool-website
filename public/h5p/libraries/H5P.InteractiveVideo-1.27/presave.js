...
H5PPresave["H5P.InteractiveVideo"] = function (content, finished) {
  ...
  if (isContentInvalid()) {
    throw new presave.exceptions.InvalidContentSemanticsException(
      "Invalid Interactive Video Error"
    )
  }
  ...
  function isContentInvalid() {
    return (
      !presave.checkNestedRequirements(
        content,
        "content.interactiveVideo.assets.interactions"
      ) || !Array.isArray(content.interactiveVideo.assets.interactions)
    )
  }
  ...
  function hasSummary() {
    return (
      presave.checkNestedRequirements(
        content,
        "content.interactiveVideo.summary.task.library"
      ) &&
      presave.checkNestedRequirements(
        content,
        "content.interactiveVideo.summary.task.params"
      )
    )
  }
}
