H5PEditor.CoursePresentation.SlideSelector = (function ($, EventDispatcher) {
  ...
  function SlideSelector(cpEditor, $slides, globalFields, slideFields, params) {
    ...
    var $contentWrapper = $(
      '<div class="h5p-background-selector-content-wrapper">'
    ).appendTo($popup)
    var $globalContent
    var $slideContent

    // Single slide semantic fields
    var singleSlideFields = H5PEditor.CoursePresentation.findField(
      "slideBackgroundSelector",
      slideFields.field.fields
    )
    ...
    var initBgSelectors = function () {
      // Global bg selector
      var templateString = H5PEditor.t(
        "H5PEditor.CoursePresentation",
        "template"
      )
      var currentSlideString = H5PEditor.t(
        "H5PEditor.CoursePresentation",
        "currentSlide"
      )
      $globalContent = createSlideSelector(templateString, true)
      globalBackground = new H5PEditor.CoursePresentation.BackgroundSelector(
        $slides.children()
      )
        .addBgSelector(globalFields, params, $globalContent, {
          isVisible: true,
        })
        .setDescription(
          H5PEditor.t("H5PEditor.CoursePresentation", "templateDescription", {
            ":currentSlide": currentSlideString,
          })
        )
        .addResetButton()
      ...
      $slides.children().each(function (idx) {
        initSingleSlide($slideContent, idx)
          ...
          .addResetButton(
            H5PEditor.t("H5PEditor.CoursePresentation", "resetToTemplate")
          )
      })
    ...
    var sanitizeSlideParams = function (idx) {
      ...
      if (
        slideParams.fillSlideBackground &&
        !slideParams.fillSlideBackground.length
      ) {
        slideParams.fillSlideBackground = undefined
      }

      if (
        slideParams.imageSlideBackground &&
        !slideParams.imageSlideBackground.path
      ) {
    ...
    var sortSlide = function (dir) {
      ...
      if (
        currentSlide + dir >= 0 &&
        currentSlide + dir < $slides.children().length
      ) {
    ...
    var initSingleSlide = function ($wrapper, idx) {
      ...
      var singleSlide = new H5PEditor.CoursePresentation.BackgroundSelector(
        $slides.children().eq(idx),
        true
      )
    ...
    var changeToSlide = function (index) {
      ...
      var $changeToSlide = singleSlides[index].getSettings()
        ? $slideContent
        : $globalContent
