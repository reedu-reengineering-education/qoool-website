H5PEditor.CoursePresentation.BackgroundSelector = (function (
  $,
  EventDispatcher
) {
  /**
  ...
  function BackgroundSelector($backgroundSlides, isSingleSlide) {
    ...
    var getTargetSlides = function () {
      return isSingleSlide
        ? $backgroundSlides
        : $backgroundSlides.filter(".global")
    }
    ...
    self.addBgSelector = function (fields, params, $wrapper, options) {
      ...
      if (
        options.index &&
        options.index > 0 &&
        options.index < $wrapper.children().length
      ) {
        $bgSelector.insertAfter(
          $wrapper.children(".single").eq(options.index - 1)
        )
      } else {
    ...
    self.addResetButton = function (text) {
      text =
        text || H5PEditor.t("H5PEditor.CoursePresentation", "resetToDefault")
