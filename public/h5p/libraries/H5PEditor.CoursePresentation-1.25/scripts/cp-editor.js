...
H5PEditor.CoursePresentation = function (parent, field, params, setValue) {
  ...
  H5P.DragNBar.FormManager.call(
    ...
    parent,
    {
      ...
      expandBreadcrumbButtonLabel: H5PEditor.t(
        "H5PEditor.CoursePresentation",
        "expandBreadcrumbButtonLabel"
      ),
      collapseBreadcrumbButtonLabel: H5PEditor.t(
        "H5PEditor.CoursePresentation",
  ...
  parent.ready(function () {
    ...
    // Active surface mode
    var activeSurfaceCheckbox = H5PEditor.findField(
      "override/activeSurface",
      parent
    )
    activeSurfaceCheckbox.on("checked", function () {
      // Make note of current height
      var oldHeight = parseFloat(
        window.getComputedStyle(that.cp.$current[0]).height
      )
...
H5PEditor.CoursePresentation.prototype = Object.create(
  H5P.DragNBar.FormManager.prototype
)
H5PEditor.CoursePresentation.prototype.constructor =
  H5PEditor.CoursePresentation
 ...
 */
H5PEditor.CoursePresentation.prototype.updateElementSizes = function (
  heightRatio
) {
 ...
 * @returns {object}
 */
H5PEditor.CoursePresentation.prototype.addElement = function (
  library,
  options
) {
  options = options || {}
  ...
  if (!elementParams) {
    ...
    if (library === "GoToSlide") {
      ...
      elementParams.action = options.action
        ? options.action
        : {
            library: library,
            params: {},
  ...
  this.cp.$boxWrapper
    .add(this.cp.$boxWrapper.find(".h5p-presentation-wrapper:first"))
    .css("overflow", "visible")

  const element = this.cp.children[slideIndex].addChild(elementParams)
  return this.cp.attachElement(
    elementParams,
    element.instance,
    this.cp.$current,
    slideIndex
...
H5PEditor.CoursePresentation.prototype.appendTo = function ($wrapper) {
  ...
  var presentationParams =
    this.parent instanceof ns.Library
      ? this.parent.params.params
      : this.parent.params
  if (
    presentationParams &&
    presentationParams.override &&
    presentationParams.override.activeSurface === true
  ) {
  ...
  var globalBackgroundField = H5PEditor.CoursePresentation.findField(
    "globalBackgroundSelector",
    this.field.fields
  )
  var slideFields = H5PEditor.CoursePresentation.findField(
    "slides",
    this.field.fields
  )
  this.backgroundSelector = new H5PEditor.CoursePresentation.SlideSelector(
    that,
    that.cp.$slidesWrapper,
    globalBackgroundField,
    slideFields,
    that.params
  ...
  var slideControls = {
    $add: H5PEditor.$(
      '<a href="#" aria-label="' +
        H5PEditor.t("H5PEditor.CoursePresentation", "newSlide") +
        '" class="h5p-slidecontrols-button h5p-slidecontrols-button-add"></a>'
    ),
    $clone: H5PEditor.$(
      '<a href="#" aria-label="' +
        H5PEditor.t("H5PEditor.CoursePresentation", "cloneSlide") +
        '" class="h5p-clone-slide h5p-slidecontrols-button h5p-slidecontrols-button-clone"></a>'
    ),
    $background: H5PEditor.$(
      '<a href="#" aria-label="' +
        H5PEditor.t("H5PEditor.CoursePresentation", "backgroundSlide") +
        '" class="h5p-slidecontrols-button h5p-slidecontrols-button-background"></a>'
    ),
    $sortLeft: H5PEditor.$(
      '<a href="#" aria-label="' +
        H5PEditor.t("H5PEditor.CoursePresentation", "sortSlide", {
          ":dir": "left",
        }) +
    ...
    $delete: H5PEditor.$(
      '<a href="#" aria-label="' +
        H5PEditor.t("H5PEditor.CoursePresentation", "removeSlide") +
        '" class="h5p-slidecontrols-button h5p-slidecontrols-button-delete"></a>'
    ),
  }
  this.slideControls = slideControls

  H5PEditor.$('<div class="h5p-slidecontrols">')
    .append([
      slideControls.$add,
      slideControls.$clone,
    ...
    .click(function () {
      var newSlide = H5P.cloneObject(
        that.params.slides[that.cp.$current.index()],
        true
      )
 ...
 */
H5PEditor.CoursePresentation.prototype.addDNBButton = function (
  library,
  options
) {
  var that = this
  ...
  return {
    ...
    createElement: function () {
      ...
      return that.addElement(
        library.uberName,
        H5P.jQuery.extend(true, {}, options)
      )
    },
  }
}
...
H5PEditor.CoursePresentation.prototype.initializeDNB = function () {
  ...
  this.$bar = H5PEditor.$(
    '<div class="h5p-dragnbar">' +
      H5PEditor.t("H5PEditor.CoursePresentation", "loading") +
      "</div>"
  ).insertBefore(this.cp.$boxWrapper)
  var slides = H5PEditor.CoursePresentation.findField(
    "slides",
    this.field.fields
  )
  var elementFields = H5PEditor.CoursePresentation.findField(
    "elements",
    slides.field.fields
  ).field.fields
  ...
  H5PEditor.LibraryListCache.getLibraries(action.options, function (libraries) {
    ...
    for (var i = 0; i < libraries.length; i++) {
      if (libraries[i].restricted !== true) {
        ...
        if (dropdownMenus[libraryId] === undefined) {
          ...
          buttons.push(
            that.addDNBButtonGroup(libraries[i], dropdownMenus[libraryId])
          )
        }
      }
    }
    ...
    var goToSlide = H5PEditor.CoursePresentation.findField(
      "goToSlide",
      elementFields
    )
    if (goToSlide) {
    ...
    that.elements.forEach(function (slide, slideIndex) {
      slide.forEach(function (element, elementIndex) {
        var elementParams =
          that.params.slides[slideIndex].elements[elementIndex]
    ...
    that.dnb.dnr.on("startResizing", function () {
      var elementParams =
        that.params.slides[that.cp.$current.index()].elements[
          that.dnb.$element.index()
        ]

      // Check for continuous text
      if (
        elementParams.action &&
        elementParams.action.library.split(" ")[0] === "H5P.ContinuousText"
      ) {
        reflowLoop = setTimeout(reflow, reflowInterval)
      }
    })
    ...
    that.dnb.dnr.on("stoppedResizing", function () {
      ...
      elementParams.width =
        that.dnb.$element.width() / (that.cp.$current.innerWidth() / 100)
      elementParams.height =
        that.dnb.$element.height() / (that.cp.$current.innerHeight() / 100)
      elementParams.y =
        (parseFloat(that.dnb.$element.css("top")) /
          that.cp.$current.innerHeight()) *
        100
      elementParams.x =
        (parseFloat(that.dnb.$element.css("left")) /
          that.cp.$current.innerWidth()) *
        100
      ...
      var elementInstance =
        that.cp.elementInstances[that.cp.$current.index()][
          that.dnb.$element.index()
        ]
    ...
    that.dnb.dnd.releaseCallback = function () {
      ...
      var element =
        that.elements[that.cp.$current.index()][that.dnb.$element.index()]

      if (that.dnb.newElement) {
        that.cp.$boxWrapper
          .add(that.cp.$boxWrapper.find(".h5p-presentation-wrapper:first"))
          .css("overflow", "")

        if (
          params.action !== undefined &&
          H5P.libraryFromString(params.action.library).machineName ===
            "H5P.ContinuousText"
        ) {
          H5P.ContinuousText.Engine.run(that)
          if (!that.params.ct) {
    ...
    that.dnb.on("paste", function (event) {
      ...
      if (pasted.from === H5PEditor.CoursePresentation.clipboardKey) {
        ...
        if (!pasted.generic) {
          ...
          if (
            pasted.generic.library.split(" ")[0] === "H5P.ExportableTextArea"
          ) {
...
H5PEditor.CoursePresentation.prototype.canPaste = function (clipboard) {
  if (clipboard) {
    if (
      clipboard.from === H5PEditor.CoursePresentation.clipboardKey &&
...
H5PEditor.CoursePresentation.prototype.validate = function () {
  ...
  for (var i = 0; i < this.elements.length; i++) {
    ...
    for (var j = 0; j < this.elements[i].length; j++) {
      ...
      var isCT =
        elementParams.action !== undefined &&
        elementParams.action.library.split(" ")[0] === "H5P.ContinuousText"
      if (isCT && !firstCT) {
...
H5PEditor.CoursePresentation.prototype.initKeywordInteractions = function () {
  ...
  that.$keywordsDNB = H5PEditor.$(
    '<ul class="h5p-dragnbar-ul h5p-dragnbar-left">' +
      ...
      '<div title="' +
      H5PEditor.t("H5PEditor.CoursePresentation", "slides") +
      '" class="h5p-dragnbar-keywords" role="button" tabindex="0">' +
      "<span>" +
      H5PEditor.t("H5PEditor.CoursePresentation", "slides") +
      "</span>" +
      "</div>" +
      ...
      "</label>" +
      '<label class="h5p-keywords-always"><input type="checkbox"/>' +
      H5PEditor.t("H5PEditor.CoursePresentation", "alwaysShow") +
      "</label>" +
      '<label class="h5p-keywords-hide"><input type="checkbox"/>' +
      H5PEditor.t("H5PEditor.CoursePresentation", "autoHide") +
      "</label>" +
      '<label class="h5p-keywords-opacity"><input type="text"/> % ' +
      H5PEditor.t("H5PEditor.CoursePresentation", "opacity") +
      "</label>" +
  ...
  var $enableKeywords = this.$bar
    .find(".h5p-keywords-enable input")
    .change(function () {
      ...
      if (that.params.keywordListEnabled) {
        if (that.params.keywordListAlwaysShow) {
          that.cp.$keywordsWrapper
            .show()
            .add(that.cp.$keywordsButton)
            .addClass("h5p-open")
          that.cp.$keywordsButton.hide()
        } else {
  ...
  var $alwaysKeywords = this.$bar
    .find(".h5p-keywords-always input")
    .change(function () {
      ...
      if (checked) {
        ...
        that.$bar
          .find(".h5p-keywords-hide input")
          .attr("checked", false)
          .attr("disabled", true)
          .parent()
          .addClass("h5p-disabled")
      } else {
        that.$bar
          .find(".h5p-keywords-hide input")
          .attr("disabled", false)
          .parent()
          .removeClass("h5p-disabled")
      }
  ...
  var $opacityKeywords = this.$bar
    .find(".h5p-keywords-opacity input")
    .change(function () {
...
H5PEditor.CoursePresentation.prototype.updateNavigationLine = function (index) {
  ...
  this.cp.slides.forEach(function (instanceArray, slideNumber) {
    ...
    if (
      that.cp.elementInstances[slideNumber] !== undefined &&
      that.cp.elementInstances[slideNumber].length
    ) {
...
H5PEditor.CoursePresentation.prototype.removeSlide = function () {
  ...
  var isRemovingDnbContainer =
    this.cp.$current.index() === this.$dnbContainer.index()

  const confirmationDialog = this.showConfirmationDialog({
    headerText: H5PEditor.t(
      "H5PEditor.CoursePresentation",
      "confirmDeleteSlide"
    ),
  ...
  confirmationDialog.on("confirmed", () => {
    ...
    if (slideKids !== undefined) {
      for (var i = 0; i < slideKids.length; i++) {
        this.removeElement(
          slideKids[i],
          slideKids[i].$wrapper,
          this.cp.elementInstances[index][i].libraryInfo &&
            this.cp.elementInstances[index][i].libraryInfo.machineName ===
              "H5P.ContinuousText"
        )
      }
    }
    ...
    // Change slide
    var move = this.cp.previousSlide()
      ? -1
      : this.cp.nextSlide(true)
        ? 0
        : undefined
 ...
 */
H5PEditor.CoursePresentation.prototype.animateNavigationLine = function (
  direction
) {
  var that = this

  var $selectedProgressPart = that.cp.$progressbar.find(
    ".h5p-progressbar-part-selected"
  )
  $selectedProgressPart.css(
    "transform",
    "translateX(" + -100 * direction + "%)"
  )

  var $selectedNext =
    direction == 1 ? $selectedProgressPart.prev() : $selectedProgressPart.next()
...
H5PEditor.CoursePresentation.prototype.updateSlidesSidebar = function () {
  ...
  $keywords.each(function (index) {
    ...
    $keyword
      .find(".h5p-keyword-subtitle")
      .html(self.cp.l10n.slide + " " + (index + 1))
    $keyword.find(".joubel-icon-edit").remove()

    var $editIcon = H5PEditor.$(
      '<a href="#" class="joubel-icon-edit h5p-hidden" title="' +
        H5PEditor.t("H5PEditor.CoursePresentation", "edit") +
        '" tabindex="0">' +
    ...
    H5PEditor.$(this)
      ...
      .blur(function (e) {
        if (
          (e.relatedTarget &&
            e.relatedTarget.className !== "joubel-icon-edit") ||
          !e.relatedTarget
        ) {
          $editIcon.addClass("h5p-hidden")
        }
 ...
 * @returns {Boolean} Indicates success.
 */
H5PEditor.CoursePresentation.prototype.sortSlide = function (
  $element,
  direction
) {
  if (!$element.length) {
  ...
  H5P.ExportableTextArea.CPInterface.changeSlideIndex(
    direction > 0 ? index : index - 1,
 ...
 */
H5PEditor.CoursePresentation.prototype.swapCollectionIndex = function (
  collection,
  firstIndex,
  secondIndex
) {
 ...
 * @param {jQuery} $other
 */
H5PEditor.CoursePresentation.prototype.swapIndexes = function (
  $current,
  $other
) {
...
H5PEditor.CoursePresentation.prototype.editKeyword = function ($span) {
  ...
  if (
    ...
    !that.params.slides[slideIndex].keywords.length
  ) {
    oldTitle = "" // Prevent editing 'No title' string
  }

  var $delete = H5PEditor.$(
    '<a href="#" class="joubel-icon-cancel" title="' +
      H5PEditor.t("H5PEditor.CoursePresentation", "cancel") +
      '">' +
      '<span class="h5p-icon-circle"></span>' +
  ...
  var $textarea = H5PEditor.$("<textarea></textarea>")
    ...
    .blur(function (event) {
      if (
        (event.relatedTarget &&
          event.relatedTarget.className !== "joubel-icon-cancel") ||
        !event.relatedTarget
      ) {
  ...
  $delete
    ...
    .blur(function (e) {
      if (
        (e.relatedTarget && e.relatedTarget.tagName !== "TEXTAREA") ||
        !e.relatedTarget
      ) {
        $textarea.blur()
      }
 ...
 */
H5PEditor.CoursePresentation.prototype.updateKeyword = function (
  keyword,
  slideIndex,
  oldTitle
) {
 ...
 * @returns {Object}
 */
H5PEditor.CoursePresentation.prototype.generateForm = function (
  elementParams,
  type
) {
  var self = this
  ...
  var elementFields = H5PEditor.$.extend(
    true,
    [],
    H5PEditor.CoursePresentation.findField("elements", slides.field.fields)
      .field.fields
  )
  ...
  if (!(library instanceof H5PEditor.None)) {
    var libraryChange = function () {
      ...
      var libraryOptions = H5PEditor.CoursePresentation.findField(
        "action",
        elementFields
      ).options
 ...
 */
H5PEditor.CoursePresentation.prototype.setImageSize = function (
  element,
  elementParams,
  fileParams
) {
  if (
    ...
    fileParams.width === undefined ||
    fileParams.height === undefined
  ) {
    return
  }
  ...
  var minSize =
    parseInt(element.$wrapper.css("font-size")) +
  ...
  var suggestedHeight =
    fileParams.height / (this.cp.$current.innerHeight() / 100)
  ...
  // Calculate new width
  elementParams.width =
    (elementParams.height * (fileParams.width / fileParams.height)) /
    this.slideRatio
}
 ...
 * @param {object} fileParams
 */
H5PEditor.CoursePresentation.prototype.setVideoSize = function (
  elementParams,
  fileParams
) {
  if (!fileParams) {
  ...
  elementParams.height =
    elementParams.width *
    (ratioParts.length === 1
      ? fileParams.aspectRatio
      : ratioParts[1] / ratioParts[0]) *
    this.slideRatio
}
 ...
 * @param {String[]} fields
 */
H5PEditor.CoursePresentation.prototype.hideFields = function (
  elementFields,
  fields
) {
 ...
 * @param {String[]} fields
 */
H5PEditor.CoursePresentation.prototype.showFields = function (
  elementFields,
  fields
) {
 ...
 */
H5PEditor.CoursePresentation.prototype.findLibraryTitle = function (
  library,
  next
) {
  var self = this
 ...
 * @returns {undefined}
 */
H5PEditor.CoursePresentation.prototype.processElement = function (
  elementParams,
  $wrapper,
  slideIndex,
  elementInstance
  ...
  if (this.elements[slideIndex][elementIndex] === undefined) {
    this.elements[slideIndex][elementIndex] = this.generateForm(
      elementParams,
      type
    )
  }
 ...
 */
H5PEditor.CoursePresentation.prototype.addToDragNBar = function (
  element,
  elementParams
) {
  var self = this

  var type = elementParams.action
    ? elementParams.action.library.split(" ")[0]
    : null
  ...
  var clipboardData = H5P.DragNBar.clipboardify(
    H5PEditor.CoursePresentation.clipboardKey,
    elementParams,
    "action"
  )
  ...
  dnbElement.contextMenu.on("contextMenuRemove", function () {
    ...
    confirmationDialog.on("confirmed", () => {
      ...
      self.removeElement(
        ...
        element.$wrapper,
        elementParams.action !== undefined &&
          H5P.libraryFromString(elementParams.action.library).machineName ===
            "H5P.ContinuousText"
      )
      self.dnb.blurAll()
    })
  ...
  dnbElement.contextMenu.on("contextMenuBringToFront", function () {
    ...
    self.cp.children[slideIndex].moveChild(
      oldZ,
      self.cp.children[slideIndex].children.length - 1
    )
  })
 ...
 * @returns {undefined}
 */
H5PEditor.CoursePresentation.prototype.removeElement = function (
  element,
  $wrapper,
  isContinuousText
) {
 ...
 * @returns {undefined}
 */
H5PEditor.CoursePresentation.prototype.showElementForm = function (
  element,
  $wrapper,
  elementParams
) {
  ...
  var machineName
  if (elementParams.action !== undefined) {
    machineName = H5P.libraryFromString(
      elementParams.action.library
    ).machineName
  }
  ...
  var focusFirstField = function () {
    ...
    if (libraryField.$myField === undefined) {
      H5P.jQuery(
        ".h5p-coursepresentation-editor .form-manager-slidein .h5peditor-text"
      )
        .eq(0)
        .focus()
    }
 ...
 */
H5PEditor.CoursePresentation.prototype.redrawElement = function (
  $wrapper,
  element,
  elementParams,
  repeat
  ...
  repeat =
    typeof repeat === "undefined" ? elements.length - 1 - elementIndex : repeat
  ...
  var $element = this.cp.attachElement(
    elementParams,
    instance,
    this.cp.$current,
    slideIndex
  ...
  if (repeat > 0) {
    repeat--
    this.redrawElement(
      elements[elementIndex].$wrapper,
      elements[elementIndex],
      elementsParams[elementIndex],
      repeat
    )
  }
}
 ...
 * @param {Object} elementParams
 */
H5PEditor.CoursePresentation.prototype.fitElement = function (
  $element,
  elementParams
) {
  var self = this
...
H5PEditor.CoursePresentation.prototype.getCTs = function (firstOnly, maxTwo) {
  ...
  for (var i = 0; i < self.elements.length; i++) {
    ...
    for (
      var j = 0;
      slideElements !== undefined && j < slideElements.length;
      j++
    ) {
      ...
      if (
        params.action !== undefined &&
        params.action.library.split(" ")[0] === "H5P.ContinuousText"
      ) {
        CTs.push({
          element: element,
          params: params,
...
H5PEditor.CoursePresentation.prototype.showConfirmationDialog = function (
  ...
  const confirmationDialog = new H5P.ConfirmationDialog(dialogOptions).appendTo(
    document.body
  )
