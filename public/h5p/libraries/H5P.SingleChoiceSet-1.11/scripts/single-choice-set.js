...
H5P.SingleChoiceSet = (function (
  ...
  function SingleChoiceSet(options, contentId, contentData) {
    ...
    if (contentData && contentData.previousState !== undefined) {
      ...
      this.userResponses =
        contentData.previousState.userResponses !== undefined
    ...
    this.$container = $("<div>", {
      class:
        "h5p-sc-set-wrapper navigatable" +
        (!this.options.behaviour.autoContinue ? " next-button-mode" : ""),
    })
  ...
  SingleChoiceSet.prototype.handleAlternativeSelected = function (event) {
    ...
    if (!this.muted) {
      ...
      SoundEffects.play(
        this.lastAnswerIsCorrect ? "positive-short" : "negative-short",
        700
      )
    }
  }
  ...
  SingleChoiceSet.prototype.handleQuestionFinished = function (event) {
    ...
    var xapiEvent = self.createXApiAnsweredEvent(
      self.options.choices[index],
      userResponse,
      duration
    )
  ...
  SingleChoiceSet.prototype.continue = function (index) {
    ...
    timeout = setTimeout(
      ...
      self.lastAnswerIsCorrect
        ? self.options.behaviour.timeoutCorrect
        : self.options.behaviour.timeoutWrong
    )
   ...
   */
  SingleChoiceSet.prototype.createXApiAnsweredEvent = function (
    question,
    userAnswer,
    duration
  ) {
  ...
  SingleChoiceSet.prototype.setScore = function (score, noXAPI) {
    ...
    var feedbackText = determineOverallFeedback(
    ...
    if (!noXAPI) {
      self.triggerXAPIScored(
        ...
        "completed",
        true,
        (100 * score) / self.options.choices.length >=
          self.options.behaviour.passPercentage
      )
    }
  ...
  SingleChoiceSet.prototype.addButtons = function () {
    ...
    if (this.options.behaviour.enableSolutionsButton) {
      this.addButton(
        "show-solution",
        this.l10n.showSolutionButtonLabel,
        function () {
          self.showSolutions()
        },
        self.results.corrects !== self.options.choices.length,
  ...
  SingleChoiceSet.prototype.getXAPIData = function () {
    ...
    var children = self.options.choices.map(function (question, index) {
      var userResponse =
        self.userResponses[index] >= 0 ? self.userResponses[index] : ""
    ...
    var xAPIEvent = XApiEventBuilder.create()
      ...
      .context(
        self.getParentAttribute("contentId"),
        self.getParentAttribute("subContentId")
      )
      .objectDefinition(definition)
      .result(result)
      .build()
   ...
   */
  SingleChoiceSet.prototype.readA11yFriendlyText = function (
    index,
    currentIndex
  ) {
    var self = this
    var correctAnswer = self.$choices
      .find(".h5p-sc-is-correct")
      [index].textContent.replace(/[\n\r]+|[\s]{2,}/g, " ")
      .trim()
    let selectedOptionText = this.lastAnswerIsCorrect
      ? self.l10n.correctText
      : self.l10n.incorrectText
    ...
    if (!self.options.behaviour.autoContinue) {
      ...
      self.$choices
        .find(".h5p-sc-current-slide .h5p-sc-is-correct .h5p-sc-a11y")
        .text(self.l10n.shouldSelect)
      self.$choices
        .find(".h5p-sc-current-slide .h5p-sc-is-wrong .h5p-sc-a11y")
        .text(self.l10n.shouldNotSelect)
      self.$choices
        .find(".h5p-sc-current-slide .h5p-sc-alternative")
        .eq(currentIndex)
        .find(".h5p-sc-a11y")
      ...
      selectedOptionText = this.lastAnswerIsCorrect
        ? self.l10n.correctText
        : self.l10n.incorrectText + correctAnswer + self.l10n.shouldSelect
    }
  ...
  H5P.JoubelUI,
  H5P.Question,
  H5P.SingleChoiceSet.SingleChoice,
  H5P.SingleChoiceSet.SolutionView,
  H5P.SingleChoiceSet.ResultSlide,
  H5P.SingleChoiceSet.SoundEffects,
  H5P.SingleChoiceSet.XApiEventBuilder,
