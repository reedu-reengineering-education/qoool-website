...
H5P.DragNResize = (function ($, EventDispatcher) {
    ...
    var addResizeHandle = function (position, corner) {
      $("<div>", {
        ...
        .mousedown(function (event) {
          that.lock =
            options && (options.lock || (corner && options.cornerLock))
  ...
  C.prototype.getElementPaddings = function () {
    return {
      horizontal:
        Number(this.$element.css("padding-left").replace("px", "")) +
        Number(this.$element.css("padding-right").replace("px", "")),
      vertical:
        Number(this.$element.css("padding-top").replace("px", "")) +
        Number(this.$element.css("padding-bottom").replace("px", "")),
    }
  }
  ...
  C.prototype.getElementBorders = function () {
    return {
      horizontal:
        Number(this.$element.css("border-left-width").replace("px", "")) +
        Number(this.$element.css("border-right-width").replace("px", "")),
      vertical:
        Number(this.$element.css("border-top-width").replace("px", "")) +
        Number(this.$element.css("border-bottom-width").replace("px", "")),
    }
  }
   ...
   */
  C.prototype.lockDimensions = function (
    isMovingWest,
    isMovingNorth,
    movesVertical,
    movesHorizontal
  ...
  C.release = function (event) {
    ...
    H5P.$window.unbind("mouseup", C.release).unbind("mousemove", C.move)
  ...
  C.prototype.toggleModifiers = function (enable) {
    this.disabledModifiers =
      enable === undefined ? !this.disabledModifiers : !enable
  }
