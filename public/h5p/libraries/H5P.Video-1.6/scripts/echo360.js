...
H5P.VideoEchoVideo = (() => {
  ...
  function EchoPlayer(sources, options, l10n) {
    ...
    placeholderElement.setAttribute(
      "style",
      "height: 100%; min-height: 200px; display: block; z-index: 100; border: none;"
    )
    ...
    const registerEchoPlayerEventListeneners = (player) => {
      ...
      player.onload = async () => {
        ...
        player.loadingPromise.then(async () => {
          ...
          const autoplayIsAllowed =
            !window.H5PEditor && (await H5P.Video.isAutoplayAllowed())
      ...
      window.addEventListener("message", (event) => {
        ...
        if (message.event === "init") {
          ...
          if (message.data.width && message.data.height) {
            ...
            ratio =
              (message.data.height + (options.controls ? CONTROLS_HEIGHT : 0)) /
              message.data.width
          }
          ...
          if (message.data.captions) {
            trackOptions = textTracks.map(
              (track) => new H5P.Video.LabelValue(track.label, track.value)
            )
          }
      ...
      if (currentState === H5P.Video.PLAYING) {
        ...
        if (previousTickMS) {
          echoUncertaintyCompensationS = Math.max(
            ...
            (time - previousTickMS + timelineUpdateDeltaSlackMS) / 1000
          )
        } else {
    ...
    const timeUpdate = (time) => {
      ...
      timeUpdateTimeout = window.setTimeout(() => {
        ...
        const delta =
          (Date.now() - this.lastTimeUpdate) * this.getPlaybackRate()
    ...
    this.getCaptionsTrack = () => {
      return (
        trackOptions.find((trackItem) => trackItem.mode === "showing") ?? null
      )
    }
