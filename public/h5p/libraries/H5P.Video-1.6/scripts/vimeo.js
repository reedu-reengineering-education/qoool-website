  ...
  function VimeoPlayer(sources, options, l10n) {
    ...
    const createVimeoPlayer = async () => {
      ...
      loadingFailedTimeout = setTimeout(() => {
        ...
        $wrapper.html(
          `<p class="vimeo-failed-loading">${l10n.vimeoLoadingError}</p>`
        )
        $wrapper.css({
          width: null,
          height: null,
    ...
    const registerVimeoPlayerEventListeneners = (player) => {
      ...
      player.on("loaded", async () => {
        ...
        try {
          ...
          /* Intentionally ignore this, and fallback on the default ratio */
        }
    ...
    const getVimeoVideoMetadata = (player) => {
      ...
      const massageVideoMetadata = (data) => {
        ...
        const tracks = data[2].reduce(
          (tracks, current) => {
            const h5pVideoTrack = new H5P.Video.LabelValue(
              current.label,
              current.language
            )
