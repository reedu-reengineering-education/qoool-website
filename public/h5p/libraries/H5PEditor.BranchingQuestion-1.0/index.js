...
H5PEditor.BranchingQuestion = (function ($, EventDispatcher, Branch) {
  function BranchingQuestionEditor(parent, field, params, setValue) {
    ...
    this.setNextContentId = function (listIndex, nextContentId) {
      var nextContentIds = this.$editor[0].querySelectorAll(
        ".field-name-nextContentId"
      )
    ...
    this.addFeedbackGroupDescription = function () {
      const feedbackGroups = this.$editor[0].querySelectorAll(
        ".field-name-feedback"
      )
      for (let i = 0; i < feedbackGroups.length; i++) {
        ...
        const feedbackDescriptionId = H5PEditor.getDescriptionId(
          H5PEditor.getNextFieldId({
            name: "feedback",
          })
        )
        ...
        description.textContent = BranchingQuestionEditor.t(
          "feedbackDescription"
        )

        feedbackGroup
          .querySelector(".title")
          .setAttribute("aria-describedby", feedbackDescriptionId)
    ...
    this.replaceContentIdWithSelector = function (addHtmlCallback) {
      ...
      for (var i = 0; i < nextContentIds.length; i++) {
        ...
        if (nextContentId.style.display === "none") {
          ...
          selectorWrapper = nextContentId.parentNode.querySelector(
            ".h5p-next-branch-wrapper"
          )
        } else {
        ...
        findList("alternatives").forEachChild(function (child, index) {
          if (index === i) {
            addHtmlCallback(
              i,
              selectorWrapper,
              H5PEditor.findField("feedback", child)
            )
          }
        })
      }
