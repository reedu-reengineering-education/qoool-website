      ...
      2: function (parameters, finished, extras) {
        ...
        for (let i = 1; i < tables.length; i++) {
          ...
          if (tables[i].includes("border")) {
            ...
            const needleEnd = tables[i].indexOf(
              '"',
              needleStart + needle.length
            )
            const borderWidth = parseInt(
              tables[i].substring(needleStart + needle.length, needleEnd)
            )
          ...
          if (tableChanges) {
            ...
            if (substring.includes('style="')) {
              tables[i] =
                substring.replace('style="', style) +
                tables[i].substring(tagEnd)
            } else {
          ...
          if (cellChanges) {
            ...
            for (let j = 1; j < headers.length; j++) {
              ...
              if (substring.includes('style="')) {
                tables[i] +=
                  substring.replace('style="', cellStyle) +
                  headers[j].substring(tagEnd)
              } else {
