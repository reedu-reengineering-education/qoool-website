  ...
  function RangeList(list) {
    ...
    self.on("rowremove", function (event) {
      ...
      if (!row.nextElementSibling) {
        ...
        setValue(
          getFirst("input", row.nextElementSibling),
          getSecond("input", row.previousElementSibling).value
        )
      }
    })
    ...
    var makeReadOnly = function (input) {
      ...
      var isFirstRow =
        !input.parentElement.parentElement.parentElement.previousElementSibling
    ...
    var validateSequence = function () {
      ...
      // Display a message
      self.messageArea.innerText = error
        ? H5PEditor.t("H5PEditor.RangeList", "rangeOutOfSequenceWarning")
        : ""
