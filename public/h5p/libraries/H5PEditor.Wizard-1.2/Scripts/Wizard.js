...
H5PEditor.widgets.wizard = H5PEditor.Wizard = (function ($, EventDispatcher) {
  ...
  C.prototype.appendTo = function ($wrapper) {
    ...
    var $panesWrapper = $('<div class="h5peditor-panes"></div>').insertBefore(
      this.$errors
    )
    ...
    var $prevButton = this.$item.find(".nav-button-prev").click(function () {
      ...
      that.showTab(
        that.$item.find("ol > li > a").eq(parseInt(currentTabId) - 1)
      )
    })
  ...
  C.prototype.updateWizardNavigation = function ($tab, currentTabId) {
    ...
    if (currentTabId > 0) {
      ...
      this.$item
        .find(".nav-button-prev .nav-button-label")
    ...
    if (currentTabId < this.$tabs.length - 1) {
      ...
      this.$item
        .find(".nav-button-next .nav-button-label")
  ...
  C.createTab = function (id, field) {
    return (
      '<li class="h5peditor-tab-li"><a href="#" class="h5peditor-tab-a h5peditor-tab-' +
      field.name.toLowerCase() +
      '" data-id="' +
      id +
      '">' +
      '<span class="field-step">' +
      C.t("step", { ":index": id + 1 }) +
      "</span>" +
