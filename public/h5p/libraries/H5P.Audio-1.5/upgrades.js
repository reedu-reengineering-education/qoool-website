    ...
    1: {
      3: function (parameters, finished, extras) {
        ...
        if (copyright) {
          ...
          var yearFrom =
            years.length > 0 ? new Date(years[0]).getFullYear() : undefined
          var yearTo =
            years.length > 0 ? new Date(years[1]).getFullYear() : undefined

          // Build metadata object
          var metadata = {
            title: copyright.title,
            authors: copyright.author
              ? [{ name: copyright.author, role: "Author" }]
              : undefined,
