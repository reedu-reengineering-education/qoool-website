export default class Scoring {
  ...
  addLibraryScore(currentId, libraryId, chosenAlternative, contentScores) {
    ...
    if (this.isBranchingQuestion(libraryParams)) {
      currentLibraryScore = this.getAlternativeScore(
        libraryParams,
        chosenAlternative
      )
      currentLibraryMaxScore = this.getQuestionMaxScore(
        libraryParams,
        chosenAlternative
      )
    } else {
    ...
    this.scores.forEach((score, index) => {
      if (score.id === currentId) {
        ...
        const isPointingToNextScore =
          this.scores.length > index + 1 &&
          this.params.content[score.id].nextContentId ===
            this.scores[index + 1].id
    ...
    if (isLoop) {
      ...
      this.scores = this.scores.filter(
        (score) => score.visitedIndex <= loopBackIndex
      )
  ...
  shouldShowScore() {
    return (
      this.params.scoringOptionGroup.scoringOption ===
