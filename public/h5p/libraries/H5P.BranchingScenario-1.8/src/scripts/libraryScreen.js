...
export default class LibraryScreen extends H5P.EventDispatcher {
  ...
  constructor(parent, courseTitle, library, previousState, lastNodeId = 0) {
    ...
    const contentTitle =
      library.type && library.type.metadata && library.type.metadata.title
        ? library.type.metadata.title
        : ""
    this.wrapper = this.createWrapper(
      courseTitle,
      contentTitle ? contentTitle : "Untitled Content",
      library.showContentTitle && contentTitle
    )
    ...
    this.currentLibraryElement = libraryWrapper.getElementsByClassName(
  ...
  createBackButton(label) {
    ...
    backButton.addEventListener("click", (event) => {
      ...
      if (
        this.currentLibraryId === 0 &&
        this.parent.params.content[this.parent.currentId].type.library.split(
          " "
        )[0] !== "H5P.BranchingQuestion"
      ) {
  ...
  handleProceed() {
    ...
    if (this.parent.navigating === false) {
      const hasFeedbackTitle =
        this.libraryFeedback.title && this.libraryFeedback.title.trim()
      const hasFeedbackSubtitle =
        this.libraryFeedback.subtitle && this.libraryFeedback.subtitle.trim()
      ...
      if (hasFeedback && this.nextLibraryId !== -1) {
        ...
        branchingQuestion.classList.add(
        ...
        const feedbackScreen = this.createFeedbackScreen(
        ...
        setTimeout(() => {
          ...
          if (
            ...
            this.currentLibraryElement.style.height =
              questionContainer.offsetHeight + "px"
        ...
        if (
          !!(hasFeedback || this.libraryFeedback.endScreenScore !== undefined)
        ) {
  ...
  createFeedbackScreen(feedback, nextContentId) {
    ...
    const proceedButtonText = this.parent.getLibrary(
  ...
  createLibraryElement(library, isNextLibrary) {
    ...
    if (
      libraryMachineName === "H5P.InteractiveVideo" ||
      libraryMachineName === "H5P.Video"
    ) {
  ...
  resetIVProgress() {
    ...
    let ivSubmitScreenStar = this.wrapper.getElementsByClassName(
      "h5p-star-foreground"
    )[0]
  ...
  appendRunnable(container, content, id) {
    ...
    if (
      ...
      (this.parent.params.content[id].forceContentFinished ===
        "useBehavioural" &&
        this.parent.params.behaviour.forceContentFinished === true)
    ) {
      this.libraryFinishingRequirements[id] = this.forceContentFinished(
        instance,
        library
      )
    ...
    if (instance) {
      try {
        if (typeof instance.pause === "function") {
          ...
          instance.elementInstances[instance.currentSlideIndex].forEach(
            (element) => {
  ...
  forceContentFinished(instance, library) {
    ...
    if (instance) {
      forceContentFinished =
        forceContentFinished || typeof instance?.getScore === "function"
    }
      ...
      case "H5P.CoursePresentation":
        ...
        instance.on("xAPI", (event) => {
          if (event.data.statement.verb.display["en-US"] === "progressed") {
            const slideProgressedTo = parseInt(
            ...
            if (
              slideProgressedTo ===
              instance.children.length + (instance.isTask ? 1 : 0)
            ) {
      ...
      case "H5P.InteractiveVideo":
        ...
        instance.video.on("stateChange", (event) => {
          if (
            ...
            const answered = instance.interactions.filter(
              (interaction) => interaction.getProgress() !== undefined
            )
  ...
  loadLibrary(library, contentId = null) {
    const loadedContentId =
      contentId !== null ? contentId : library.nextContentId
    ...
    if (library.type.library.split(" ")[0] !== "H5P.BranchingQuestion") {
      ...
      if (nextLibrary?.type.library.split(" ")[0] !== "H5P.BranchingQuestion") {
        this.nextLibraries[loadedContentId] = this.createLibraryElement(
          nextLibrary,
          true
        )
    ...
    // If it is a branching question, load all the possible libraries
      ...
      const alternatives =
        library.type.params.branchingQuestion.alternatives || []
      ...
      alternatives
        ...
        .forEach((nextContentId) => {
          ...
          if (
            ...
            this.nextLibraries[nextContentId] = this.createLibraryElement(
              nextLibrary,
              true
            )
  ...
  hasValidVideo(currentLibraryParams) {
    ...
    let videoSource =
      videoLibrary === "Interactive Video"
  ...
  hideFeedbackDialogs() {
    ...
    const questionWrapper = wrapper.querySelector(
      ".h5p-branching-question-wrapper"
    )
    if (questionWrapper) {
  ...
  showNextLibrary(
    ...
    if (!LibraryScreen.isBranching(library)) {
      ...
      this.libraryTitle.innerHTML =
        library.showContentTitle && contentTitle ? contentTitle : "&nbsp;"
      ...
      const done = () => {
        ...
        this.currentLibraryElement = libraryWrapper.getElementsByClassName(
          "h5p-branching-scenario-content"
        )[0] // TODO: Why no use 'libraryElement' ?
      ...
      const libraryElement = libraryWrapper.getElementsByClassName(
        "h5p-branching-scenario-content"
      )[0]
      ...
      const labelId =
        "h5p-branching-question-title-" + LibraryScreen.idCounter++
      const questionContainer = branchingQuestion.querySelector(
      ...
      questionContainer.classList.add("h5p-fly-in")
      branchingQuestion.querySelector(".h5p-branching-question-title").id =
        labelId
      const multiChoiceWrapper = branchingQuestion.querySelector(
      ...
      const isSmallerDevice =
        this.parent.container.classList.contains("h5p-mobile-screen")
      ...
      branchingQuestion.addEventListener("animationend", () => {
        const firstAlternative = branchingQuestion.querySelectorAll(
  ...
  hideBackgroundFromReadspeaker() {
    ...
    h5pContainer.querySelectorAll(selector).forEach((element) => {
      if (element instanceof HTMLMediaElement) {
        ...
        const currentTabindex = element.classList.contains("ui-slider-handle")
          ? 0
    ...
    if (this.tabbables) {
      this.tabbables.forEach((tabbable) => {
        ...
        if (element instanceof HTMLMediaElement) {
          ...
          tabbable.tabindex !== undefined &&
          tabbable.tabindex !== null
        ) {
