  ...
  constructor(params, contentId, extras = {}) {
    ...
    this.on("navigated", (event) => {
      ...
      if (!this.params.preventXAPI) {
        ...
        const isBranchingQuestionAndAnswered =
          isBranching &&
      ...
      if (this.currentEndScreen?.isShowing) {
        if (nextLibrary) {
          if (!LibraryScreen.isBranching(nextLibrary)) {
            ...
            this.libraryScreen.show({ skipAnimation: event.data.isResuming })
          }
        } else {
          // Showing two end screens after each other
          this.libraryScreen.hideFeedbackDialogs()
          this.currentEndScreen.hide({ skipAnimation: event.data.isResuming })
          this.currentEndScreen = null
        }
      ...
      if (
         ...
         */
        const wrapper =
          this.libraryScreen.wrapper.querySelector(".h5p-slide-in")
