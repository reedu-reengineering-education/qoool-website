  ...
  C.prototype.setActive = function () {
    ...
    for (i = 0; i < this.field.options.length; i++) {
      ...
      if (this.field.multiple) {
        ...
        html +=
          '<li><label class="h5p-editor-label"><input type="checkbox" value="' +
          option.value +
          '"' +
          (selected ? ' checked="checked"' : "") +
          '/><div class="h5p-label-text">' +
          option.label +
          "</div></label></li>"
      } else {
        // Check if selected
        ...
        html +=
          '<li><label class="h5p-editor-label"><input type="radio" name="dynboxradio1" value="' +
          option.value +
          '"' +
          (selected ? ' checked="checked"' : "") +
          "/>" +
    ...
    this.$item.html(
      html
        ? '<div class="h5peditor-label">' +
            ...
            "</div>" +
            (this.field.multiple
              ? '<a href="#" class="h5p-selectall">' +
                H5PEditor.t("H5PEditor.DragQuestion", "selectAll") +
                "</a>"
              : "") +
            '<ul class="h5peditor-dynamiccheckboxes-select">' +
            html +
            "</ul>"
        : ""
    )
    ...
    var updateSelectAll,
      $a,
      $checkboxes = this.$item.find("input").change(function () {
        that.change($(this))
    ...
    updateSelectAll = function () {
      if ($checkboxes.length) {
        if ($checkboxes.length === $checkboxes.filter(":checked").length) {
          $a.addClass("h5p-deselectall").text(
            H5PEditor.t("H5PEditor.DragQuestion", "deselectAll")
          )
        } else {
          $a.removeClass("h5p-deselectall").text(
            H5PEditor.t("H5PEditor.DragQuestion", "selectAll")
          )
        }
      }
