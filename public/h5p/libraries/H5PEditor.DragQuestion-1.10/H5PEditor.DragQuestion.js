...
H5PEditor.widgets.dragQuestion = H5PEditor.DragQuestion = (function (
  ...
  function C(parent, field, params, setValue) {
    ...
    this.backgroundOpacity =
      ...
      this.backgroundOpacity.trim() === ""
        ? undefined
        : this.backgroundOpacity

    // Update opacity for all dropzones/draggables when global background opacity is changed
    parent.ready(function () {
      H5PEditor.findField("../behaviour/backgroundOpacity", parent)
        .$item.find("input")
        .on("change", function () {
          that.backgroundOpacity = $(this).val().trim()
          that.backgroundOpacity =
            that.backgroundOpacity === "" ? undefined : that.backgroundOpacity
          that.updateAllElementsOpacity(
            that.elements,
            that.params.elements,
            "element"
          )
        })
    })
    ...
    if (typeof this.elementLibraryOptions[0] === "object") {
      this.elementLibraryOptions = this.elementLibraryOptions.map(
        function (option) {
          return option.name
        }
      )
  ...
  C.prototype.supported = function (lib) {
    for (var i = 0; i < this.libraries.length; i++) {
      if (
        this.libraries[i].restricted !== true &&
        this.libraries[i].uberName === lib
      ) {
  ...
  C.prototype.createHtml = function () {
    ...
    html +=
      ...
      '<div class="h5peditor-dragquestion">' +
      C.t("noTaskSize") +
      "</div>" +
      '<div class="h5peditor-fluid-dialog">' +
      '  <div class="h5peditor-fd-inner"></div>' +
      '  <div class="h5peditor-fd-buttons">' +
      '    <a href="#" class="h5peditor-fd-button h5peditor-done">' +
      C.t("done") +
      "</a>" +
      '    <a href="#" class="h5peditor-fd-button h5peditor-remove">' +
      C.t("remove") +
      "</a>" +
      "  </div>" +
      "</div>"
    ...
    if (this.field.description !== undefined) {
      html +=
        '<div class="h5peditor-field-description">' +
  ...
  C.prototype.setActive = function () {
    ...
    if (this.dnb === undefined) {
      this.$editor
        .html(
          '<div class="h5p-throbber">' +
            H5PEditor.t("core", "loading") +
            "</div>"
        )
        .addClass("h5p-ready")
      H5PEditor.LibraryListCache.getLibraries(
        this.elementLibraryOptions,
        function (libraries) {
  ...
  C.prototype.resize = function () {
    ...
    this.pToEm =
      parseFloat(window.getComputedStyle(this.$editor[0]).width) /
      this.fontSize /
      100
  }
  ...
  C.prototype.activateEditor = function (libraries) {
    ...
    // Create new bar
    this.dnb = new DragNBar(
      this.getButtons(buttonLibraries),
      this.$editor,
      this.$item,
      { libraries: libraries }
    )
    ...
    this.dnb.stopMovingCallback = function (x, y) {
      ...
      var params = that.dnb.dnd.$element.hasClass("h5p-dq-dz")
        ? that.params.dropZones[id]
        : that.params.elements[id]
      params.x = x
      params.y = y
    }
    ...
    this.dnb.on("paste", function (event) {
      ...
      if (pasted.from === clipboardKey) {
        ...
        elementParams.width =
          (pasted.width || elementParams.width / that.pToEm) * that.pToEm
        elementParams.height =
          (pasted.height || elementParams.height / that.pToEm) * that.pToEm
    ...
    this.dnb.dnr.on("stoppedResizing", function (dimensions) {
      ...
      var params = that.dnb.$element.hasClass("h5p-dq-dz")
        ? that.params.dropZones[id]
        : that.params.elements[id]
      ...
      if (dimensions.data.left !== undefined) {
        params.x =
          dimensions.data.left / (parseFloat(containerStyle.width) / 100)
      }
      if (dimensions.data.top !== undefined) {
        params.y =
          dimensions.data.top / (parseFloat(containerStyle.height) / 100)
      }
  ...
  C.prototype.generateForm = function (semantics, params) {
    ...
    if ($lib.length !== 0) {
      $lib
        .children("label, select, .h5peditor-field-description")
        .hide()
        .end()
        .children(".libwrap")
        .css("margin-top", "0")
  ...
  C.prototype.insertElement = function (index) {
    ...
    element.$element = $(
      '<div class="h5p-dq-element" style="width:' +
        elementParams.width +
        "em;height:" +
        elementParams.height +
        "em;top:" +
      ...
      .hover(
        function () {
          C.setElementOpacity(
            element.$element,
            that.getElementOpacitySetting(elementParams)
          )
        },
        function () {
    ...
    setTimeout(function () {
      var type = elementParams.type
        ? elementParams.type.library.split(" ")[0]
        : null

      var dnbElement = that.dnb.add(
        element.$element,
        DragNBar.clipboardify(clipboardKey, elementParams, "type"),
        {
      ...
      dnbElement.contextMenu.on(
        "contextMenuRemove",
        that.elementRemove.bind(that, element)
      )
      dnbElement.contextMenu.on(
        "contextMenuBringToFront",
        that.elementBringToFront.bind(that, element)
      )
      dnbElement.contextMenu.on(
        "contextMenuSendToBack",
        that.elementSendToBack.bind(that, element)
      )
      ...
      that.params.dropZones.forEach(function (dropZone) {
        ...
        for (let i = 0; i < dropZone.correctElements.length; i++) {
          if (dropZone.correctElements[i] === value) {
            ...
            dropZone.correctElements[i] =
              "" + (parseInt(dropZone.correctElements[i]) - 1)
          }
        }
      })
  ...
  C.prototype.editElement = function (element) {
    ...
    var disableOpacityField = !!(
      that.params.elements[id].dropZones.length !== 0 && this.backgroundOpacity
    )
    H5PEditor.findField("backgroundOpacity", element)
      .$item.find("input")
      .prop({
  ...
  C.prototype.updateElement = function (element, id) {
    ...
    var type =
      params.type.library.split(" ")[0] === "H5P.AdvancedText"
        ? "text"
        : "image"
    var hasCk =
      element.children[0].children !== undefined &&
      element.children[0].children[0].ckeditor !== undefined
    if (type === "text" && hasCk) {
      ...
      element.instance = H5P.newRunnable(
        {
          ...
          params: {
            text: params.type.params.text.replace(
              /\*([^*]+)\*/g,
              '<span class="h5p-dragquestion-placeholder">$1</span>'
            ),
          },
        },
        H5PEditor.contentId,
      ...
      params.type.params.text = params.type.params.text.replace(
        /\*([^*]+)\*/g,
        "$1"
      )
    ...
    var label =
      type === "text"
        ? $("<div>" + params.type.params.text + "</div>").text()
        : params.type.params.alt + ""
    ...
    if (params.dropZones !== undefined && params.dropZones.length) {
      ...
      if (type === "text" && hasCk) {
        ...
        this.hideDialogCallback = function () {
          ...
          element.$element
            .find(".h5p-dragquestion-placeholder")
            .each(function () {
              var $span = $(this)
  ...
  C.prototype.insertDropZone = function (index) {
    ...
    dropZoneParams.type = dropZoneParams.type || {
      library: that.fakeDropzoneLibrary,
    }

    dropZone.$dropZone = $(
      '<div class="h5p-dq-dz" style="width:' +
        dropZoneParams.width +
        "em;height:" +
        dropZoneParams.height +
        "em;top:" +
        dropZoneParams.y +
        "%;left:" +
        dropZoneParams.x +
    ...
    setTimeout(function () {
      var dropzoneDnBElement = that.dnb.add(
        dropZone.$dropZone,
        DragNBar.clipboardify(clipboardKey, dropZoneParams, "type")
      )
      ...
      const removeDropzoneDialogActions = function (confirmFlag) {
        ...
        that.elementFields[that.elementDropZoneFieldWeight].options.splice(
          id,
          1
        )
      ...
      dropzoneDnBElement.contextMenu.on("contextMenuBringToFront", function () {
        ...
        var options =
          that.elementFields[that.elementDropZoneFieldWeight].options
  ...
  C.prototype.updateInternalDropZoneIDs = function (start) {
    for (var i = start; i < this.dropZones.length; i++) {
      ...
      this.elementFields[this.elementDropZoneFieldWeight].options[i].value =
        i + ""
    }
  }
  ...
  C.prototype.editDropZone = function (dropZone) {
    ...
    var options = (this.dropZoneFields[
      this.dropZoneElementFieldWeight
    ].options = [])
    var dropZones
  ...
  C.prototype.updateDropZone = function (dropZone, id) {
    ...
    if (params.showLabel === true) {
      $('<div class="h5p-dq-dz-label">' + params.label + "</div>").appendTo(
        dropZone.$dropZone
      )
    ...
    if (
      params.tipsAndFeedback !== undefined &&
      params.tipsAndFeedback.tip !== undefined &&
      params.tipsAndFeedback.tip.trim().length !== 0
    ) {
      dropZone.$dropZone.append(
        H5P.JoubelUI.createTip(params.tipsAndFeedback.tip, {
    ...
    C.setOpacity(
      dropZone.$dropZone.add(dropZone.$dropZone.children(".h5p-dq-dz-label")),
      "background",
   ...
   * @param {String} type
   */
  C.prototype.updateAllElementsOpacity = function (
    domElements,
    elements,
    type
  ) {
    ...
    for (var i = 0; i < domElements.length; i++) {
      C.setElementOpacity(
        domElements[i]["$" + type],
        this.getElementOpacitySetting(elements[i])
      )
    }
  }
  ...
  C.prototype.getElementOpacitySetting = function (element) {
    if (
      (element.dropZones !== undefined && element.dropZones.length === 0) ||
      this.backgroundOpacity === undefined
    ) {
  ...
  C.setAlphas = function (style, prefix, alpha) {
    ...
    while (colorStart !== -1) {
      ...
      var channels = style
        .substring(colorStart + prefix.length, colorEnd)
        .split(",")

      // Set alpha channel
      channels[3] =
        channels[3] !== undefined ? parseFloat(channels[3]) * alpha : alpha

      style =
        style.substring(0, colorStart) +
        "rgba(" +
        channels.join(",") +
        style.substring(colorEnd, style.length)
