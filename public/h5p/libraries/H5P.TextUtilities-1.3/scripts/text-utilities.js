...
H5P.TextUtilities = (function () {
  ...
  TextUtilities.isIsolated = function (candidate, text, params) {
    ...
    var delimiter =
      !!params && !!params.delimiter
        ? params.delimiter
        : TextUtilities.WORD_DELIMITER

    var pos =
      !!params && !!params.index && typeof params.index === "number"
        ? params.index
    ...
    var succ =
      pos + candidate.length === text.length
        ? ""
        : text[pos + candidate.length].replace(delimiter, "")
  ...
  TextUtilities.areSimilar = function (string1, string2) {
    ...
    var levenshtein = H5P.TextUtilities.computeLevenshteinDistance(
      string1,
      string2,
      true
    )
   ...
   */
  TextUtilities.computeLevenshteinDistance = function (
    str1,
    str2,
    countSwapping
  ) {
   ...
   * @returns {number} Distance.
   */
  TextUtilities.computeJaroDistance = function (
    str1,
    str2,
    favorSameStart,
    longTolerance
    ...
    if (favorSameStart) {
      if (distance > 0.7 && str1Len > 3 && str2Len > 3) {
        ...
        if (longTolerance) {
          if (
            Math.max(str1Len, str2Len) > 4 &&
            matches > l + 1 &&
            2 * matches >= Math.max(str1Len, str2Len) + l
          ) {
            distance +=
              (1.0 - distance) *
              ((matches - l - 1) / (str1Len + str2Len - 2 * l + 2))
          }
        }
      }
    }
  ...
  TextUtilities.fuzzyFind = function (needle, haystack, params) {
    ...
    if (
      ...
      typeof params.windowSize !== "number"
    ) {
      params = { windowSize: 3 }
    }
    ...
    for (var i = 0; i < haystack.length - needle.length + 1; i++) {
      ...
      for (var j = 0; j < hay.length; j++) {
        if (
          TextUtilities.isIsolated(hay[j], haystack) &&
          TextUtilities.areSimilar(hay[j], needle)
        ) {
          match = hay[j]
          found = true
          break
