  ...
  ImageHotspots.Popup = function (
    ...
    if (fullscreen) {
      ...
      popupWidth = toTheLeft
        ? x - hotspotWidth - pointerWidthInPercent
        : 100 - popupLeft
    }
    ...
    if (!fullscreen) {
      this.$pointer = $("<div/>", {
        class:
          "h5p-image-hotspot-popup-pointer to-the-" +
          (toTheLeft ? "left" : "right") +
          (legacy ? " legacy-positioning" : ""),
      })
        .css({
          top: y + "%",
        })
    ...
    self.show = function () {
      if (!fullscreen) {
        ...
        self.$pointer.css({
          left: toTheLeft ? popupWidth + "%" : popupLeft + "%",
        })
      }
