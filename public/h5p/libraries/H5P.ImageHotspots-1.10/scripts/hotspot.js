...
;(function ($, ImageHotspots) {
   ...
   */
  ImageHotspots.Hotspot = function (
    config,
    options,
    id,
    isSmallDeviceCB,
    ...
    var iconImageExists =
      options.iconImage !== undefined && options.iconType === "image"

    if (this.config.content === undefined || this.config.content.length === 0) {
      throw new Error(
        "Missing content configuration for hotspot. Please fix in editor."
      )
    }
    ...
    this.$element = $("<button/>", {
      ...
      tabindex: 0,
      "aria-haspopup": true,
      html: iconImageExists
        ? `<img src="${H5P.getPath(options.iconImage.path, this.id)}" alt=""/>`
        : "",
      click: function () {
  ...
  ImageHotspots.Hotspot.prototype.showPopup = function (focusPopup) {
    ...
    this.config.content.forEach(function (action) {
      ...
      if (machineName === "H5P.Audio") {
        if (
          actionInstance.audio &&
          actionInstance.params.playerMode === "full" &&
          !!window.chrome
        ) {
          // Workaround for missing https://github.com/h5p/h5p-audio/pull/48
          actionInstance.audio.style.height = "54px"
        } else if (
          actionInstance.$audioButton &&
          actionInstance.params.playerMode === "transparent"
        ) {
    ...
    if (!waitForLoaded.length) {
      ...
      self.actionInstances.length === 1 &&
      self.actionInstances[0].libraryInfo.machineName === "H5P.Image"
    ) {
    ...
    $("body")
      .children()
      .on("click.h5p-image-hotspot-popup", function (event) {
        var $target = $(event.target)
        if (
          self.visible &&
          !$target.hasClass("h5p-enable-fullscreen") &&
          !$target.hasClass("h5p-disable-fullscreen") &&
   ...
   */
  ImageHotspots.Hotspot.prototype.setTrapFocusTo = function (
    hotspot,
    trapReverseTab
  ) {
    this.$element.on("keydown.trapfocus", function (e) {
      var keyCombination =
        e.which === 9 && (trapReverseTab ? e.shiftKey : !e.shiftKey)
      if (keyCombination) {
        hotspot.focus()
        e.stopPropagation()
        return false
  ...
  ImageHotspots.Hotspot.prototype.setTitle = function (title) {
    ...
    this.$element.attr(
      "aria-label",
      hasAudioVideo
        ? `${title}. ${this.options.containsAudioVideoLabel}`
        : title
    )
  }
