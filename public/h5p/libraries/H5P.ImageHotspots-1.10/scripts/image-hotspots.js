...
H5P.ImageHotspots = (function ($, EventDispatcher) {
  ...
  ImageHotspots.prototype.attach = function ($container) {
    ...
    if (this.options.image === null || this.options.image === undefined) {
      $container.append(
        '<div class="background-image-missing">Missing required background image</div>'
      )
      return
    }
    ...
    if (this.options.image && this.options.image.path) {
      ...
      if (this.options.backgroundImageAltText) {
        this.$image.attr(
          "alt",
          this.massageAttributeOutput(this.options.backgroundImageAltText)
        )
      } else {
    ...
    for (var i = 0; i < numHotspots; i++) {
      try {
        var hotspot = new ImageHotspots.Hotspot(
          this.options.hotspots[i],
          this.options,
          this.id,
          isSmallDevice,
          self
        ...
        var hotspotTitle = this.options.hotspots[i].header
          ? this.options.hotspots[i].header
  ...
  ImageHotspots.prototype.toggleTrapFocus = function (enable) {
    ...
    if (enable) {
      ...
      if (this.hotspots.length > 1) {
        ...
        this.hotspots[0].setTrapFocusTo(
          this.hotspots[this.hotspots.length - 1],
          true
        )
      }
    } else {
  ...
  ImageHotspots.prototype.resize = function (e) {
    ...
    var height = Math.floor(
      (width / self.options.image.width) * self.options.image.height
    )
    ...
    if (this.isRoot() && H5P.isFullscreen) {
      ...
      if (!forceImageHeight && height > containerHeight) {
        ...
        width = Math.floor(
          (height / self.options.image.height) * self.options.image.width
        )
      }
    ...
    self.isSmallDevice =
      containerWidth / parseFloat($("body").css("font-size")) < 40
  }
