...
H5PEditor.widgets.duration = H5PEditor.Duration = (function ($) {
  ...
  C.prototype.createHtml = function () {
    ...
    const descriptionId =
      this.field.description !== undefined
        ? H5PEditor.getDescriptionId(id)
        : undefined
    var input =
      H5PEditor.createText(
        this.params !== undefined
          ? C.humanizeTime(this.params.from)
          : undefined,
        15,
        "From",
        id,
      ...
      " - " +
      H5PEditor.createText(
        this.params !== undefined ? C.humanizeTime(this.params.to) : undefined,
        15,
        "To",
        undefined,
        descriptionId
  ...
  C.prototype.validate = function () {
    ...
    this.$inputs.each(function (i) {
      ...
      if (
        (that.field.optional === undefined || !that.field.optional) &&
        !value.length
      ) {
        that.$errors.append(
          H5PEditor.createError(
      ...
      if (values.length !== 2 && values.length !== 3) {
        that.$errors.append(
          H5PEditor.createError(C.t("invalidTime", { ":property": field.name }))
        )
        return false
      }
      ...
      if (values[j] !== undefined) {
        ...
        if (
          ...
          that.$errors.append(
            H5PEditor.createError(
              C.t("invalidTime", { ":property": field.name })
            )
          )
          return false
        }
      ...
      if (field.max !== undefined && value > field.max) {
        that.$errors.append(
          H5PEditor.createError(
            H5PEditor.t("core", "exceedsMax", {
              ":property": field.name,
              ":max": C.humanizeTime(field.max),
            })
          )
        )
        return false
