H5P.TrueFalse = (function ($, Question) {
  ...
  function TrueFalse(options, id, contentData) {
    ...
    TrueFalse.counter =
      TrueFalse.counter === undefined ? 0 : TrueFalse.counter + 1
    ...
    // The radio group
    var answerGroup = new H5P.TrueFalse.AnswerGroup(
      domId,
      params.correct,
      params.l10n
    )
    if (
      ...
      contentData.previousState.answer !== undefined
    ) {
    ...
    var registerButtons = function () {
      ...
      if (params.behaviour.enableSolutionsButton === true) {
        self.addButton(
          ...
          params.l10n.showSolutionButton,
          function () {
    ...
    var addResponseToXAPI = function (xAPIEvent) {
      ...
      xAPIEvent.setScoredResult(
        isCorrect ? MAX_SCORE : 0,
        MAX_SCORE,
        self,
        true,
        isCorrect
      )
      xAPIEvent.data.statement.result.response = isCorrect
        ? getCorrectAnswer()
        : getWrongAnswer()
    }
    ...
    var toggleButtonState = function (state) {
      toggleButtonVisibility(
        Button.SHOW_SOLUTION,
        state === State.FINISHED_WRONG
      )
      toggleButtonVisibility(Button.CHECK, state === State.ONGOING)
      toggleButtonVisibility(
        Button.TRYAGAIN,
        state === State.FINISHED_WRONG || state === State.INTERNAL_SOLUTION
      )
    }
    ...
    var checkAnswer = function () {
      ...
      toggleButtonState(
        score === MAX_SCORE ? State.FINISHED_CORRECT : State.FINISHED_WRONG
      )
    ...
    self.registerDomElements = function () {
      ...
      self.setIntroduction(
        '<div id="' + domId + '">' + params.question + "</div>"
      )
    ...
    self.getTitle = function () {
      return H5P.createTitle(
        self.contentData &&
          self.contentData.metadata &&
          self.contentData.metadata.title
          ? self.contentData.metadata.title
          : "True-False"
      )
    }
    ...
    self.showSolutions = function (internal) {
      ...
      toggleButtonState(
        internal ? State.INTERNAL_SOLUTION : State.EXTERNAL_SOLUTION
      )
    }
    ...
    self.addResponseToXAPI = function (xAPIEvent) {
      ...
      if (self.getCurrentState().answer !== undefined) {
        currentResponse += answerGroup.isCorrect()
          ? getCorrectAnswer()
          : getWrongAnswer()
      }
