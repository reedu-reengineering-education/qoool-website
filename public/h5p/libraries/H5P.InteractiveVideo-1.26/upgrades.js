    ...
    1: {
      ...
      7: function (parameters, finished) {
        ...
        if (
          ...
          for (i = 0; i < interactions.length; i++) {
            ...
            interaction.displayType =
              interaction.displayAsButton === undefined ||
              interaction.displayAsButton
                ? "button"
                : "poster"
            ...
            if (
              interaction.action &&
              interaction.action.library &&
              interaction.action.library.split(" ")[0] === "H5P.Link"
            ) {
      ...
      11: function (parameters, finished) {
        if (parameters.interactiveVideo) {
          ...
          if (videoParams) {
            ...
            videoParams.startScreenOptions =
              videoParams.startScreenOptions || {}
            videoParams.advancedSettings.startScreenOptions =
              videoParams.startScreenOptions
            videoParams.advancedSettings.startScreenOptions.poster =
              videoParams.poster
          ...
          if (
            ...
            for (var i = 0; i < interactions.length; i++) {
              ...
              if (
                ...
                if (
                  ["H5P.Text", "H5P.Image", "H5P.Table"].indexOf(lib) !== -1
                ) {
                  interaction.visuals = {
      ...
      12: function (parameters, finished) {
        function moveOldStartScreenOptions(video) {
          ...
          video.startScreenOptions.poster =
            video.startScreenOptions.startScreenOptions.poster
          video.startScreenOptions.hideStartTitle =
            video.startScreenOptions.startScreenOptions.hideStartTitle
          video.startScreenOptions.shortStartDescription =
            video.startScreenOptions.startScreenOptions.shortStartDescription
      ...
      20: function (parameters, finished, extras) {
        ...
        if (
          parameters &&
          parameters.interactiveVideo &&
          parameters.interactiveVideo.video &&
          parameters.interactiveVideo.video.startScreenOptions
        ) {
          title = parameters.interactiveVideo.video.startScreenOptions.title
          copyright =
            parameters.interactiveVideo.video.startScreenOptions.copyright
        }
        ...
        extras.metadata.title = title
          ? title.replace(/<[^>]*>?/g, "")
          : extras.metadata.title
            ? extras.metadata.title
            : "Interactive Video"
        extras.metadata.licenseExtras = copyright
          ? (copyright = copyright.replace(/<[^>]*>?/g, ""))
          : extras.metadata.licenseExtras
            ? extras.metadata.licenseExtras
            : undefined
