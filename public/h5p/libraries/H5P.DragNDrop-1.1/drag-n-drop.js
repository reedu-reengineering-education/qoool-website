...
H5P.DragNDrop.prototype.press = function ($element, x, y) {
  ...
  H5P.$body
    ...
    .css({
      ...
      "-webkit-user-select":
        "none" /*, 'user-select': 'none', '-ms-user-select': 'none'*/,
    })
    .attr("unselectable", "on")[0].onselectstart = H5P.$body[0].ondragstart =
    function () {
      return false
    }
  ...
  this.marginX =
    parseInt($element.css("marginLeft")) + parseInt($element.css("marginRight"))
  this.marginY =
    parseInt($element.css("marginTop")) + parseInt($element.css("marginBottom"))
...
H5P.DragNDrop.prototype.move = function (x, y) {
  if (!this.moving) {
    if (
      this.startMovingCallback !== undefined &&
      !this.startMovingCallback(x, y)
    ) {
      return
    }
  ...
  if (transformCSSTranslateXYArray[0] != "") {
    transformCSSTranslateX = parseInt(
      transformCSSTranslateXYArray[0].match(/-?\d+/g)
    )
    transformCSSTranslateY = parseInt(
      transformCSSTranslateXYArray[1].match(/-?\d+/g)
    )
  }

  if (theElement.style.left.includes("%")) {
    left =
      (this.containerWidth * parseInt(theElement.style.left)) / 100 +
      transformCSSTranslateX
  } else {
    left = parseInt(theElement.style.left) + transformCSSTranslateX
  }
  if (theElement.style.top.includes("%")) {
    top =
      (this.containerHeight * parseInt(theElement.style.top)) / 100 +
      transformCSSTranslateY
  } else {
  ...
  const topRightCorner0DegreesPos = [
    origin[0] + 0.5 * width,
    origin[1] - 0.5 * height,
  ]
  const topLeftCorner0DegreesPos = [
    origin[0] - 0.5 * width,
    origin[1] - 0.5 * height,
  ]

  const angleTopRight0Degrees =
    (Math.atan2(
      origin[1] - topRightCorner0DegreesPos[1],
      topRightCorner0DegreesPos[0] - origin[0]
    ) *
      180) /
    Math.PI
  const angleTopLeft0Degrees =
    (Math.atan2(
      origin[1] - topLeftCorner0DegreesPos[1],
      topLeftCorner0DegreesPos[0] - origin[0]
    ) *
      180) /
    Math.PI
  ...
  const hypToCorners = Math.sqrt(
    Math.pow(width / 2, 2) + Math.pow(height / 2, 2)
  )

  const newPosTopRightCorner = this.findNewPoint(
    origin[0],
    origin[1],
    angleTopRight0Degrees - angle,
    hypToCorners
  )
  const newPosTopleftCorner = this.findNewPoint(
    origin[0],
    origin[1],
    angleTopLeft0Degrees - angle,
    hypToCorners
  )
  const newPosBottomRightCorner = this.findNewPoint(
    origin[0],
    origin[1],
    angleBottomRight0Degrees - angle,
    hypToCorners
  )
  const newPosBottomLeftCorner = this.findNewPoint(
    origin[0],
    origin[1],
    angleBottomLeft0Degrees - angle,
    hypToCorners
  )

  const rightmostPoint = Math.max(
    newPosTopRightCorner[0],
    newPosTopleftCorner[0],
    newPosBottomLeftCorner[0],
    newPosBottomRightCorner[0]
  )
  ...
  const topmostPoint = Math.min(
    newPosTopRightCorner[1],
    newPosTopleftCorner[1],
    newPosBottomLeftCorner[1],
    newPosBottomRightCorner[1]
  )
  ...
  var posX =
    x - this.containerOffset.left + this.scrollLeft + leftmostPointAdjust
  ...
  var startX =
    this.startX - this.adjust.x - this.containerOffset.left + this.scrollLeft
  var startY =
    this.startY - this.adjust.y - this.containerOffset.top + this.scrollTop
  if (!this.snap && (posX !== startX || posY !== startY)) {
    this.trigger("showTransformPanel")
  } else if (this.snap) {
    var xChanged =
      Math.round(posX / this.snap) * this.snap !==
      Math.round(startX / this.snap) * this.snap
    var yChanged =
      Math.round(posY / this.snap) * this.snap !==
  ...
  this.$element.css({
    left: posX - transformCSSTranslateX,
    top: posY - transformCSSTranslateY,
  })
...
H5P.DragNDrop.prototype.findNewPoint = function (
  originX,
  originY,
  angle,
  distance
...
H5P.DragNDrop.release = function (event) {
  ...
  H5P.$body
    .css({
      "-moz-user-select": "",
      "-webkit-user-select": "" /*, 'user-select': '', '-ms-user-select': ''*/,
    })
    .removeAttr("unselectable")[0].onselectstart = H5P.$body[0].ondragstart =
    null
