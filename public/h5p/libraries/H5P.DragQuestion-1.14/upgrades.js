    ...
    1: {
      1: {
        contentUpgrade: function (parameters, finished) {
          ...
          parameters.behaviour = {
            enableRetry:
              parameters.enableTryAgain === undefined
                ? true
                : parameters.enableTryAgain,
            preventResize:
              parameters.preventResize === undefined
                ? true
                : parameters.preventResize,
            singlePoint:
      ...
      4: function (parameters, finished) {
        if (
          ...
          parameters.question.task.elements !== undefined
        ) {
      ...
      11: function (parameters, finished) {
        ...
        if (
          ...
          for (var i = 0; i < dropZones.length; i++) {
            ...
            var tip =
              dropZone !== undefined &&
              dropZone.tip !== undefined &&
              typeof dropZone.tip === "string"
                ? dropZone.tip
                : ""
        ...
        if (
          parameters.question !== undefined &&
          parameters.question.settings !== undefined
        ) {
          parameters.behaviour.dropZoneHighlighting =
            parameters.question.settings.dropZoneHighlighting
          parameters.behaviour.autoAlignSpacing =
            parameters.question.settings.autoAlignSpacing
          parameters.behaviour.enableFullScreen =
            parameters.question.settings.enableFullScreen
      ...
      13: function (parameters, finished, extras) {
        ...
        if (parameters.question && parameters.question.settings) {
          ...
          if (parameters.behaviour) {
            parameters.behaviour.showTitle =
              parameters.question.settings.showTitle || false
          }

          metadata.title =
            parameters.question.settings.questionTitle ||
      ...
      14: function (parameters, finished, extras) {
        ...
        if (taskParams.dropZones && taskParams.elements) {
          ...
          dropZones.forEach((dropzone, index) => {
            dropzone.correctElements = dropzone.correctElements.filter(
              (draggableId) => {
                // Check for existence first
                const draggableExists =
                  draggables.length > parseInt(draggableId)
                if (!draggableExists) {
                  return false
                }
                ...
                const canBeDropped = draggableParams.dropZones.includes(
                  index.toString()
                )
                return canBeDropped
              }
