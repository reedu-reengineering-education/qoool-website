...
H5PPresave["H5P.DragQuestion"] = function (content, finished) {
  ...
  if (correctDropZones.length === 0 || isSinglePoint()) {
    ...
    score = content.question.task.elements
      .filter(function (element, index) {
        return (
          Array.isArray(correctDropZones[index]) && correctDropZones.length > 0
        )
      })
  ...
  function hasDropZones() {
    return (
      ...
      ) && Array.isArray(content.question.task.dropZones)
    )
  }
  ...
  function hasElements() {
    return (
      ...
      ) && Array.isArray(content.question.task.elements)
    )
  }
