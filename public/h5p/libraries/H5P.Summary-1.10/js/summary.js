H5P.Summary = (function ($, Question, XApiEventBuilder, StopWatch) {
  ...
  function Summary(options, contentId, contentData) {
    ...
    this.getScore = function () {
      ...
      return self.summaries
        ? self.summaries.reduce(function (result, panel, index) {
  ...
  Summary.prototype.createQuestion = function () {
    ...
    for (var panelIndex = 0; panelIndex < that.summaries.length; panelIndex++) {
      if (
        !(
          that.summaries[panelIndex].summary &&
          that.summaries[panelIndex].summary.length
        )
      ) {
        continue
      }
      ...
      for (
        var summaryIndex = 0;
        summaryIndex < that.summaries[panelIndex].summary.length;
        summaryIndex++
      ) {
        ...
        if (
          that.answers[panelIndex] &&
          that.answers[panelIndex].indexOf(id) !== -1
        ) {
    ...
    var $summary_list = $(
      '<ul role="list" aria-labelledby="answerListHeading-' +
        that.summaryId +
        '"></ul>'
    )
    ...
    var $evaluation_content = $(
      '<div id="questionDesc-' +
        that.summaryId +
        '" class="summary-evaluation-content">' +
        that.options.intro +
        "</div>"
    )
    ...
    var $progress = $(
      '<div class="summary-progress" aria-live="polite" role="status"></div>'
    )
    var $progressNumeric = $(
      '<div class="summary-progress-numeric" aria-hidden="true"></div>'
    )
    ...
    var selectedAlt = function ($el, setFocus) {
      ...
      if (that.answer[nodeId]) {
        ...
        $progressNumeric.html(
          that.options.solvedLabel +
            " " +
            (panelId + 1) +
            "/" +
            that.summaries.length
        )
        ...
        const label = that.options.alternativeIncorrectLabel + ". " + $el.text()
    ...
    for (var i = 0; i < elements.length; i++) {
      ...
      if (i < that.progress) {
        ...
        for (var j = 0; j < element.summaries.length; j++) {
          ...
          if (that.answer[sum.id]) {
            $summary_list.append(
              '<li style="display:block">' + sum.text + "</li>"
            )
            $summary_container.addClass("has-results")
            break
          }
      ...
      var $page = $(
        '<ul aria-labelledby="questionDesc-' +
          that.summaryId +
          '" role="radiogroup" class="h5p-panel" data-panel="' +
          i +
          '"></ul>'
      )
      ...
      if (
        ...
        element.tip.trim().length > 0
      ) {
        $evaluation_content.append(
          H5P.JoubelUI.createTip(element.tip, {
            tipLabel: that.options.tipButtonLabel,
          })
        )
      }
      ...
      for (var j = 0; j < element.summaries.length; j++) {
        ...
        var $node = $(
          "" +
            '<li role="radio" aria-checked="false" data-name="' +
            j +
            '" data-bit="' +
            element.summaries[j].id +
            '" class="' +
            summaryLineClass +
            '">' +
    ...
    if (that.progress === elements.length) {
      ...
      // Show first panel
      $(".h5p-panel:eq(" + that.progress + ")", that.$myDom).css({
        display: "block",
      })
      if (that.progress) {
        that.offset =
          $(
            ".summary-claim-unclicked:visible:first",
            that.$myDom
          ).outerHeight() * that.errorCounts.length
      }
    }
  ...
  Summary.prototype.gotoPreviousOption = function (that, currentFocusedOption) {
    ...
    var prevRadioEle = $(
      "ul[data-panel=" + that.progress + "] li[role='radio']",
      this.$myDom
    )
  ...
  Summary.prototype.doFinalEvaluation = function () {
    ...
    var summary = H5P.Question.determineOverallFeedback(
      that.options.overallFeedback,
      percent / 100
    )
      .replace("@score", score)
    ...
    var scoreBarLabel = that.options.scoreBarLabel
      .replace(":num", score)
      .replace(":total", maxScore)
  ...
  Summary.prototype.storeUserResponse = function (questionIndex, answerIndex) {
    ...
    self.userResponses[questionIndex].push(
      this.dataBitMap[questionIndex][answerIndex]
    )
  }
   ...
   */
  Summary.prototype.createXApiAnsweredEvent = function (
    panel,
    userAnswer,
    panelIndex,
    duration
  ...
  Summary.prototype.getXAPIData = function () {
    ...
    var children = self.summaries.map(function (panel, index) {
      ...
      var event = self.createXApiAnsweredEvent(
        panel,
        userResponse,
        index,
        duration
