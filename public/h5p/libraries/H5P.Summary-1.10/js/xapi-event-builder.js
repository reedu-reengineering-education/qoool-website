...
H5P.Summary.XApiEventBuilder = (function ($, EventDispatcher) {
  ...
  XApiEventDefinitionBuilder.prototype = Object.create(
   ...
   * @return {XApiEventDefinitionBuilder}
   */
  XApiEventDefinitionBuilder.prototype.interactionType = function (
    interactionType
  ) {
   ...
   * @return {XApiEventDefinitionBuilder}
   */
  XApiEventDefinitionBuilder.prototype.correctResponsesPattern = function (
  ...
  XApiEventDefinitionBuilder.prototype.build = function () {
    ...
    setAttribute(
      definition,
      "description",
      localizeToEnUS(this.attributes.description)
    )
    setAttribute(definition, "interactionType", this.attributes.interactionType)
    setAttribute(
      definition,
      "correctResponsesPattern",
      this.attributes.correctResponsesPattern
    )
    setAttribute(
      definition,
      "type",
    ...
    if (isDefined(this.attributes.rawScore)) {
      ...
      if (isDefined(this.attributes.maxScore) && this.attributes.maxScore > 0) {
        ...
        setAttribute(
          ...
          "scaled",
          Math.round(
            (this.attributes.rawScore / this.attributes.maxScore) * 10000
          ) / 10000
        )
      }
    }
  ...
  XApiEventBuilder.prototype.build = function () {
      ...
      event.data.statement.context = {
        contextActivities: {
          parent: [
            {
              id: getContentXAPIId(
                this.attributes.parentContentId,
                this.attributes.parentSubContentId
              ),
              objectType: "Activity",
            },
          ],
    ...
    event.data.statement.object = {
      id: getContentXAPIId(
        this.attributes.contentId,
        this.attributes.subContentId
      ),
      objectType: "Activity",
    }
    ...
    setAttribute(
      event.data.statement.object,
      "definition",
      this.attributes.objectDefinition
    )

    // sets h5p specific attributes
    if (
      event.data.statement.object.definition &&
      (this.attributes.contentId || this.attributes.subContentId)
    ) {
