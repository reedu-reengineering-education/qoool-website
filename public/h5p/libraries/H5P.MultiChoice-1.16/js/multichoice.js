...
H5P.MultiChoice = function (options, contentId, contentData) {
  ...
  self.registerDomElements = function () {
    ...
    for (var i = 0; i < params.answers.length; i++) {
      params.answers[i].checkboxOrRadioIcon = getCheckboxOrRadioIcon(
        params.behaviour.singleAnswer,
        params.userAnswers.indexOf(i) > -1
      )
    }

    // Register Introduction
    self.setIntroduction(
      '<div id="' + params.labelId + '">' + params.question + "</div>"
    )
    ...
    for (let i = 0; i < params.answers.length; i++) {
      ...
      $("<li>", {
        ...
        "data-id": i,
        html:
          '<div class="h5p-alternative-container"><span class="h5p-alternative-inner">' +
          answer.text +
          "</span></div>",
        appendTo: $myDom,
      })
    ...
    var $answers = $(".h5p-answer", $myDom).each(function (i) {
      ...
      $multichoiceTip
        .click(function () {
          ...
          var openFeedback = !$tipContainer
            .children(".h5p-feedback-dialog")
            .is($feedbackDialog)
    ...
    if (!params.behaviour.singleAnswer) {
      ...
      if (
        params.userAnswers.length &&
        params.answers[params.userAnswers[0]].correct
      ) {
        score = 1
      } else {
        score = 0
  ...
  this.showAllSolutions = function () {
    ...
    $myDom
      ...
      .each(function (i, e) {
        ...
        const className =
          "h5p-solution-icon-" +
          (params.behaviour.singleAnswer ? "radio" : "checkbox")
      ...
      .find(".h5p-question-plus-one, .h5p-question-minus-one")
      .remove()
  ...
  this.getMaxScore = function () {
    return !params.behaviour.singleAnswer && !params.behaviour.singlePoint
      ? calculateMaxScore()
      : params.weight
  }
  ...
  var addButtons = function () {
    ...
    self.addButton(
      "show-solution",
      params.UI.showSolutionButton,
      function () {
        if (
          params.behaviour.showSolutionsRequiresInput &&
          !self.getAnswerGiven(true)
        ) {
    ...
    // Check button
    if (
      params.behaviour.enableCheckButton &&
      (!params.behaviour.autoCheck || !params.behaviour.singleAnswer)
    ) {
      self.addButton(
        "check-answer",
        params.UI.checkAnswerButton,
        function () {
          self.answered = true
          checkAnswer()
          $myDom.find(".h5p-answer:first-child").focus()
      ...
      function () {
        ...
        if (params.behaviour.randomAnswers) {
          ...
          for (i = 0; i < answersDisplayed.length; i++) {
            ...
            $(answersDisplayed[i])
              .find(".h5p-alternative-inner")
              .html(params.answers[i].text)
            $(tip[i])
              .detach()
              .appendTo(
                $(answersDisplayed[idMap.indexOf(oldIdMap[i])]).find(
  ...
  var getFeedbackText = function (score, max) {
    ...
    var feedback = H5P.Question.determineOverallFeedback(
  ...
  this.showCheckSolution = function (skipFeedback) {
    ...
    $myDom.find(".h5p-answer").each(function (i, e) {
      ...
      if (!skipFeedback) {
        if (
          chosen &&
          a.tipsAndFeedback.chosenFeedback !== undefined &&
          a.tipsAndFeedback.chosenFeedback !== ""
        ) {
          addFeedback($e, a.tipsAndFeedback.chosenFeedback)
        } else if (
          !chosen &&
          a.tipsAndFeedback.notChosenFeedback !== undefined &&
          a.tipsAndFeedback.notChosenFeedback !== ""
        ) {
  ...
  var calcScore = function () {
    ...
    if (params.behaviour.singlePoint) {
      score =
        (100 * score) / calculateMaxScore() >= params.behaviour.passPercentage
          ? params.weight
          : 0
    }
  }
    ...
    for (var i = 0; i < params.answers.length; i++) {
      ...
      if (params.answers[i].correct) {
        if (!params.singleAnswer) {
          ...
          definition.correctResponsesPattern[0] +=
            params.answers[i].originalOrder
        } else {
          definition.correctResponsesPattern.push(
            "" + params.answers[i].originalOrder
          )
  ...
  var addResponseToXAPI = function (xAPIEvent) {
    ...
    for (var i = 0; i < params.userAnswers.length; i++) {
      ...
      response +=
        idMap === undefined
          ? params.userAnswers[i]
          : idMap[params.userAnswers[i]]
    }
  ...
  H5P.MultiChoice.counter =
    H5P.MultiChoice.counter === undefined ? 0 : H5P.MultiChoice.counter + 1
