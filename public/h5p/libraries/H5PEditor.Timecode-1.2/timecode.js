...
H5PEditor.widgets.timecode = H5PEditor.Timecode = (function ($) {
  ...
  function Timecode(parent, field, params, setValue) {
    ...
    var createHtml = function () {
      ...
      const descriptionId =
        self.field.description !== undefined
          ? H5PEditor.getDescriptionId(id)
          : undefined
      return H5PEditor.createFieldMarkup(
        field,
        H5PEditor.createText(params, 15, undefined, id, descriptionId),
        id
      )
    }
      ...
      try {
        if (!value.length) {
          ...
          if (field.max !== undefined && value > field.max) {
            throw ct("exceedsMax", {
              ":property": field.name,
              ":max": toTimecode(field.max),
            })
          } else if (field.min !== undefined && value < field.min) {
            throw ct("exceedsMin", {
              ":property": field.name,
              ":min": toTimecode(field.min),
            })
          }
        }
      } catch (error) {
