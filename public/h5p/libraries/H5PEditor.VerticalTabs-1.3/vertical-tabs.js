...
H5PEditor.VerticalTabs = (function ($) {
  ...
  function VerticalTabs(list) {
    ...
    var moveItem = function ($item, $placeholder, x, y) {
      ...
      if (
        $next.length &&
        y + $item.height() > $next.offset().top + $next.height() / 2
      ) {
    ...
    var toggleOrderButtonsState = function () {
      $tabs.children().each(function (index, element) {
        ...
        $tab
          .find(".order-up")
          .attr("aria-disabled", isTopTab)
          .attr("tabindex", isTopTab ? "-1" : "0")
        var isBottomTab = !$tab.next().length
        $tab
          .find(".order-down")
          .attr("aria-disabled", isBottomTab)
          .attr("tabindex", isBottomTab ? "-1" : "0")
      })
    }
    ...
    self.addItem = function (item) {
       ...
       */
      var up = function () {
        H5P.$window.unbind("mousemove", move).unbind("mouseup", up)

        H5P.$body.attr("unselectable", "off").css({
          "-moz-user-select": "",
          "-webkit-user-select": "",
          "user-select": "",
          "-ms-user-select": "",
      ...
      var down = function (event) {
        ...
        H5P.$window.mousemove(move).mouseup(up)

        // Start tracking mouse
        H5P.$body.attr("unselectable", "on").css({
      ...
      $("<div/>", {
        class: "h5p-vtab-a",
        html:
          '<span class="h5p-index-label">' +
          ($tab.index() + 1) +
          '</span><span>.</span> <span class="h5p-label" title="' +
          entity +
          '">' +
          entity +
          "</span>",
      ...
      H5PEditor.createButton(
        "order-up",
        H5PEditor.t("core", "orderItemUp"),
        moveItemUp
      ).appendTo($orderWrapper)
      H5PEditor.createButton(
        "order-down",
        H5PEditor.t("core", "orderItemDown"),
        moveItemDown
      ).appendTo($orderWrapper)

      // Create confirmation dialog for removing list item
      var confirmRemovalDialog = new H5P.ConfirmationDialog({
        dialogText: H5PEditor.t("core", "confirmRemoval", {
          ":type": entity.toLocaleLowerCase(),
        }),
      ...
      if (item instanceof H5PEditor.Group) {
        ...
        if (item.params.metadata && item.params.metadata.title) {
          ...
          summary =
            decodeEntity(item.params.metadata.title) +
            (!item.libraries ||
            (item.libraries.length > 1 &&
              item.params.metadata.title.indexOf(summary) === -1)
              ? " (" + summary + ")"
              : "")
        }
        setTabLabel(summary)
        ...
        const setSummary = function () {
          if (
            ...
            item.params.metadata.title
          ) {
            // The given title usually makes more sense than the type name
            setTabLabel(
              decodeEntity(item.params.metadata.title) +
                (item.libraries.length > 1 &&
                item.params.metadata.title.indexOf(lastLib.title) === -1
                  ? " (" + lastLib.title + ")"
                  : "")
            )
          } else {
        ...
        var change = function () {
          ...
          setTabLabel(
            value === "-"
              ? entity
              : item.$select.children('option[value="' + value + '"]').text()
          )
        }
