...
H5P.MarkTheWords.Word = (function () {
  ...
  function Word($word, params) {
    ...
    function removeDoubleAsterisks(wordString) {
      ...
      while (asteriskIndex !== -1) {
        if (wordString.indexOf("*", asteriskIndex + 1) === asteriskIndex + 1) {
          slicedWord =
            wordString.slice(0, asteriskIndex) +
            wordString.slice(asteriskIndex + 2, input.length)
        }
    ...
    function handleAsterisks() {
      ...
      while (asteriskIndex !== -1) {
        handledInput =
          handledInput.slice(0, asteriskIndex) +
          handledInput.slice(asteriskIndex + 1, handledInput.length)
    ...
    this.markClear = function () {
      $word.attr("aria-selected", false).removeAttr("aria-describedby")
    ...
    this.markCheck = function (scorePoints) {
      if (this.isSelected()) {
        $word.attr(
          "aria-describedby",
          isAnswer ? Word.ID_MARK_CORRECT : Word.ID_MARK_INCORRECT
        )
