;(function (DragNBar, EventDispatcher) {
  ...
  DragNBar.FormManager = function (parent, l10n, customIconClass) {
    ...
    const initialize = function () {
      ...
      self.formContainer = (
        self.isMainLibrary ? parent.$form : parent.$libraryWrapper
      )[0]
      ...
      breadcrumbButton = createButton(
        "breadcrumb-menu",
        l10n.expandBreadcrumbButtonLabel,
      ...
      self.formButtons.appendChild(
        createButton("delete", l10n.deleteButtonLabel, function () {
      ...
      // Create 'Done' button
      self.formButtons.appendChild(
        createButton("done", l10n.doneButtonLabel, function () {
          formTargets[formTargets.length - 1].trigger(
            "formdone",
            formTargets.length
          )
      ...
      // Footer form buttons
      self.footerFormButtons.appendChild(
        createButton("done", l10n.doneButtonLabel, function () {
          formTargets[formTargets.length - 1].trigger(
            "formdone",
            formTargets.length
      ...
      if (self.isMainLibrary && H5PEditor.semiFullscreen !== undefined) {
        ...
        const fullscreenButton = createButton("fullscreen", "", function () {
          if (manager.exitSemiFullscreen) {
            ...
            manager.exitSemiFullscreen = H5PEditor.semiFullscreen(
      ...
      self.on("validate", function () {
        if (
          parent.metadata &&
          (!parent.metadata.title || !H5P.trim(parent.metadata.title))
        ) {
    ...
    const createTitles = function (libraryField, customTitle, customIconId) {
      const library =
        libraryField.params && libraryField.params.library
          ? libraryField.params.library
          : libraryField.currentLibrary
            ? libraryField.currentLibrary
            : undefined
      ...
      const setTitle = function (title) {
        textWrapper.innerText =
          menuTitleText.innerText =
          tooltip.innerText =
          menuTitleTooltip.innerText =
            title
      }
      ...
      const getTitle = function () {
        if (customTitle) {
          ...
          (libraryField.params &&
            libraryField.params.metadata &&
            libraryField.params.metadata.title &&
            libraryField.params.metadata.title.substr(0, 8) !== "Untitled") ||
          (libraryField.metadata &&
            libraryField.metadata.title &&
    ...
    const closeForm = function () {
      ...
      previousBreadcrumb.removeEventListener(
        "keypress",
        handleBreadcrumbKeypress
      )
      ...
      if (
        activeSubForm.previousSibling.classList.contains("form-manager-form")
      ) {
      ...
      activeSubForm.style.height =
        manager.formContainer.getBoundingClientRect().height - headHeight + "px"
    ...
    self.openForm = function (
      ...
      subForm.style.height =
        manager.formContainer.getBoundingClientRect().height -
      ...
      manager.formBreadcrumbMenu.insertBefore(
        titles.menu,
        manager.formBreadcrumbMenu.firstChild
      )
        ...
        subForm.style.marginLeft =
          parseFloat(
            window.getComputedStyle(
              manager.formContainer.children[
                manager.formContainer.children.length - 2
              ]
            ).marginLeft
          ) -
          20 +
    ...
    self.toggleBreadcrumbMenu = function () {
      if (self.formContainer.classList.contains("mobile-menu-open")) {
        ...
        breadcrumbButton.children[0].innerText =
          l10n.expandBreadcrumbButtonLabel
        breadcrumbButton.setAttribute(
          "aria-label",
          l10n.expandBreadcrumbButtonLabel
        )
        ...
        breadcrumbButton.children[0].innerText =
          l10n.collapseBreadcrumbButtonLabel
        breadcrumbButton.setAttribute(
          "aria-label",
          l10n.collapseBreadcrumbButtonLabel
        )
    ...
    self.updateFormResponsiveness = function () {
      ...
      const updateActiveTooltips = function (element) {
        ...
        for (let i = 0; i < element.children.length; i++) {
          ...
          if (
            breadcrumbTitle.firstChild.offsetWidth &&
            breadcrumbTitle.firstChild.scrollWidth >
              breadcrumbTitle.firstChild.offsetWidth + 1
          ) {
