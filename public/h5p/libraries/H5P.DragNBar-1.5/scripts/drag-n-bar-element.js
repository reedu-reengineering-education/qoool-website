...
H5P.DragNBarElement = (function ($, ContextMenu, EventDispatcher) {
  ...
  function DragNBarElement(dragNBar, clipboardData, options) {
    ...
    if (!this.options.disableContextMenu) {
      this.contextMenu = new ContextMenu(
        this.dnb.$dialogContainer,
        this,
        this.options.hasCoordinates,
        this.options.disableResize,
        this.options.disableCopy,
        this.options.directionLock
      )
    }
    this.focused = false
  ...
  DragNBarElement.prototype.resizeContextMenu = function (left) {
    ...
    var paddingLeft = Number(
      this.contextMenu.$parent.css("padding-left").replace("px", "")
    )
  ...
  DragNBarElement.prototype.blur = function () {
    if (this.$element) {
      ...
      if (!this.options.disableContextMenu) {
        ...
        this.contextMenu.trigger("contextMenuTransform", {
