...
H5P.DragNBarContextMenu = (function ($, EventDispatcher) {
  ...
  function ContextMenu(
    ...
    this.buttons = [
      ...
      {
        name: "BringToFront",
        label: H5PEditor.t("H5P.DragNBar", "bringToFrontLabel"),
      },
      {
        name: "SendToBack",
        label: H5PEditor.t("H5P.DragNBar", "sendToBackLabel"),
      },
    ...
    if (!disableCopy) {
      this.buttons.splice(1, 0, {
        name: "Copy",
        label: H5PEditor.t("H5P.DragNBar", "copyLabel"),
      })
    }
    ...
    self.on("contextMenuTransform", function (e) {
      ...
      self.$transformButtonWrapper.toggleClass(
        "active",
        self.showingTransformPanel
      )
      ...
      .on("change keydown", function (event) {
        if (event.type === "change" || event.which === 13) {
          ...
          if (!isNaN(x) && !isNaN(y)) {
            ...
            var max = {
              x:
                self.dnb.$container.width() -
                self.dnbElement.getElement().outerWidth(),
              y:
                self.dnb.$container.height() -
                self.dnbElement.getElement().outerHeight(),
            }
  ...
  ContextMenu.prototype.addDimensions = function () {
    ...
    var updateDimensions = function (type) {
      ...
      var containerSize = parseFloat(
        window.getComputedStyle(self.dnb.$container[0])[type]
      )
      var elementStyle = window.getComputedStyle($element[0])
      var max =
        containerSize -
        parseFloat(elementStyle[type === "width" ? "left" : "top"])
      ...
      var padding =
        $element[0].getBoundingClientRect()[type] -
        parseFloat(elementStyle[type])
      target -= padding
    ...
    self.$width = self.getNewInput(
      "width",
      H5PEditor.t("H5P.DragNBar", "widthLabel"),
      self.$dimensions,
      updateDimensions,
    ...
    self.$height = self.getNewInput(
      "height",
      H5PEditor.t("H5P.DragNBar", "heightLabel"),
      self.$dimensions,
      updateDimensions,
  ...
  ContextMenu.prototype.addTransform = function (enableTransform) {
    var self = this
    var transformButtonObject = {
      name: "Transform",
      label: H5PEditor.t("H5P.DragNBar", "transformLabel"),
    }
    ...
    // Attach button
    if (enableTransform) {
      self.createButton(transformButtonObject).appendTo($transformButtonWrapper)
    }
  ...
  ContextMenu.prototype.updateDimensions = function () {
    ...
    var paddingX =
      ...
      parseFloat(elementSize["width"])
    var paddingY =
      $element[0].getBoundingClientRect()["height"] -
      parseFloat(elementSize["height"])
   ...
   * @returns {H5P.jQuery}
   */
  ContextMenu.prototype.getNewInput = function (
    type,
    label,
    $container,
    handler,
  ...
  ContextMenu.prototype.removeFromMenu = function (buttonName) {
    var $removeButton = this.$buttons.children(
      ".h5p-context-menu-button-" + buttonName
    )
    $removeButton.remove()
  }
  ...
  ContextMenu.prototype.updateContextMenu = function () {
    ...
    this.addTransform(enableTransform).appendTo(this.$contextMenu)
