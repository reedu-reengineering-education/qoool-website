/*global ns*/
H5PEditor.widgets.radioSelector = H5PEditor.RadioSelector = (function (
  $,
  EventDispatcher
) {
  var idCounter = 0
  ...
  function RadioSelector(parent, field, params, setValue) {
    ...
    var $options = $('<div class="h5p-radio-selector-options">').appendTo(
      $container
    )

    // Wrapper for content corresponding to radio buttons
    var $values = $('<div class="h5p-radio-selector-values">').appendTo(
      $container
    )
      ...
      self.children.forEach(function (child, idx) {
        ...
        if (child.params) {
          ...
          if (child instanceof ns.File) {
            ...
            child instanceof H5PEditor.ColorSelector &&
            child.params.length
          ) {
            type = "bgColor"
            value = child.params
    ...
    self.resetCheckedOption = function () {
      ...
      if (resetOption instanceof ns.File) {
        ...
        $values.children().eq(currentOption).find(".file > a.remove").click()
