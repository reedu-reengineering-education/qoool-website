...
H5PEditor.widgets.interactiveVideo = H5PEditor.InteractiveVideo = (function (
  ...
  InteractiveVideoEditor.prototype = Object.create(
  ...
  InteractiveVideoEditor.prototype.canPaste = function (clipboard) {
    if (clipboard) {
      if (
        clipboard.from === InteractiveVideoEditor.clipboardKey &&
   ...
   */
  InteractiveVideoEditor.prototype.setChooserItem = function (
    $chooser,
    textIndex,
    selector,
    action
  ...
  InteractiveVideoEditor.prototype.setActive = function () {
    ...
    this.IV.on("controls", function () {
      ...
      // Add DragNBar.
      that.$bar = $(
        '<div class="h5p-interactive-video-dragnbar">' + t("loading") + "</div>"
        ...
        function (libraries) {
          ...
          const allowedLibraries = libraries.filter(function (library) {
            return library.name !== "H5P.FreeTextQuestion" ||
              H5PEditor.reportingIsAvailable === undefined
              ? true
              : H5PEditor.reportingIsAvailable
          })
  ...
  InteractiveVideoEditor.prototype.showInteractionTitle = function (
    title,
    $interaction
  ) {
    ...
    if (totalOffsetX < 0) {
      ...
      totalOffsetX =
        this.IV.$videoWrapper.width() - this.$interactionTitle.outerWidth(true)
    }
    var totalOffsetY =
   ...
   */
  InteractiveVideoEditor.prototype.createInteractionForm = function (
    interaction,
    parameters
  ) {
    var self = this
   ...
   * @param {Object} parameters
   */
  InteractiveVideoEditor.prototype.processInteraction = function (
    interaction,
    parameters
  ) {
    var self = this
    ...
    var eligibleForRequireCompletion =
      InteractiveVideoEditor.XAPI_QUESTION_TYPES.filter(
   ...
   */
  InteractiveVideoEditor.prototype.getInteractionFields = function (
    interaction
  ) {
   ...
   * @param {object} newParams
   */
  InteractiveVideoEditor.prototype.setImageSize = function (
    parameters,
    newParams
  ) {
    if (
   ...
   */
  InteractiveVideoEditor.prototype.setLibraryName = function (
    $form,
    libraryType
  ) {
   ...
   */
  InteractiveVideoEditor.prototype.showConfirmationDialog = function (
    dialogOptions,
    handleActions
  ) {
   ...
   * @param interaction
   */
  InteractiveVideoEditor.prototype.openInteractionDialog = function (
    interaction
  ) {
    var that = this
    if (
   ...
   */
  InteractiveVideoEditor.prototype.addInteractionToDnb = function (
    interaction,
    $interaction,
    options
  ) {
   ...
   */
  InteractiveVideoEditor.prototype.newInteraction = function (
    interaction,
    $interaction
  ) {
    var that = this
   ...
   */
  InteractiveVideoEditor.prototype.fit = function (
    $interaction,
    interactionParams
  ) {
    var self = this
   ...
   * @returns {H5P.jQuery}
   */
  InteractiveVideoEditor.prototype.addInteraction = function (
    library,
    options
  ) {
  ...
  InteractiveVideoEditor.prototype.startGuidedTour = function (force) {
    if (InteractiveVideoEditor.showGuidedTour) {
      H5PEditor.InteractiveVideo.GuidedTours.start(
