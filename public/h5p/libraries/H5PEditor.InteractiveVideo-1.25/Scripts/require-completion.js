...
H5PEditor.InteractiveVideo.RequireCompletion = (function () {
  ...
  function RequireCompletion(IVEditor, interaction) {
     ...
     */
    var adaptivityFields = IVEditor.getInteractionFields(
      interactionFields.adaptivity
    )
    ...
    interactionFields.pause.$item
      .get(0)
      .parentNode.insertBefore(
        conflictingTimeMsg,
        interactionFields.pause.$item.get(0).nextSibling
      )
    ...
    function insertRetryButtonMessage(library) {
      ...
      if (enableRetry && !enableRetryDisabledMsg.parentNode) {
        enableRetry.$item
          .get(0)
          .parentNode.insertBefore(
            enableRetryDisabledMsg,
            enableRetry.$item.get(0).nextSibling
          )
      }
    }
    ...
    function handleConflictingInteractions(interaction, otherInteractions) {
      ...
      var conflictingInteractions = getConflictingInteractions(
        interaction,
        otherInteractions
      )
      conflictingTimeMsg.classList.toggle(
        "h5p-hide",
        !conflictingInteractions.length
      )
    }
    ...
    function hasDurationWithin(time, interaction) {
      ...
      return (
        (time.from >= duration.from && time.from <= duration.to) ||
    ...
    function togglePause(isChecked) {
      ...
      interactionFields.pause.$item.toggleClass(
        "h5p-has-disabled-msg",
        isChecked
      )
