...
H5P.Blanks = (function ($, Question) {
  ...
  function Blanks(params, id, contentData) {
    ...
    this.labelId =
      "h5p-blanks-instructions-" + Blanks.idCounter + "-" + H5P.createUUID()
    ...
    self.setIntroduction(
      '<div id="' + this.labelId + '">' + self.params.text + "</div>"
    )
  ...
  Blanks.prototype.registerButtons = function () {
    ...
    if (
      !self.params.behaviour.autoCheck &&
  ...
  Blanks.prototype.handleBlanks = function (question, handler) {
    ...
    while (clozeStart !== -1 && clozeEnd !== -1) {
      ...
      var clozeContent = question
        .substring(clozeStart, clozeEnd)
        .replaceAll(/<\/?[a-z]*\d?>/gi, "")
      var replacer = ""
  ...
  Blanks.prototype.createQuestions = function () {
    ...
    for (var i = 0; i < self.params.questions.length; i++) {
      ...
      question = self.handleBlanks(question, function (solution) {
        ...
        var defaultUserAnswer =
          self.params.userAnswers.length > self.clozes.length
            ? self.params.userAnswers[self.clozes.length]
            : null
        var cloze = new Blanks.Cloze(
          solution,
          self.params.behaviour,
          defaultUserAnswer,
          {
    ...
    self.$questions[0].insertBefore(
      self.a11yHeader,
      this.$questions[0].childNodes[0] || null
    )
    ...
    this.$questions
      ...
      .each(function (i) {
        ...
        if (self.params.behaviour.autoCheck) {
          afterCheck = function () {
            ...
            self.read(
              (this.correct()
                ? self.params.answerIsCorrect
                : self.params.answerIsWrong
              ).replace(":ans", answer)
            )
      ...
      .keydown(function (event) {
        ...
        var tabPressedAutoCheck =
          event.keyCode === 9 && self.params.behaviour.autoCheck

        if (enterPressed || tabPressedAutoCheck) {
          // Figure out which inputs are left to answer
          $inputs = self.$questions.find(
            ".h5p-input-wrapper:not(.h5p-correct) .h5p-text-input"
          )
        ...
        if (
          (tabPressedAutoCheck && isLastInput && !self.shiftPressed) ||
  ...
  Blanks.prototype.toggleButtonVisibility = function (state) {
    ...
    if (this.params.behaviour.enableRetry) {
      if (
        (state === STATE_CHECKING && !allCorrect) ||
  ...
  Blanks.prototype.removeMarkedResults = function () {
    this.$questions
      .find(".h5p-input-wrapper")
      .removeClass("h5p-correct h5p-wrong")
  ...
  Blanks.prototype.getxAPIDefinition = function () {
    ...
    for (var i = 0; i < this.params.questions.length; i++) {
      ...
      let question = this.params.questions[i].replaceAll(
        /_{10,}/gi,
        "_________"
      )
    ...
    definition.extensions[XAPI_CASE_SENSITIVITY] =
      this.params.behaviour.caseSensitive
  ...
  Blanks.prototype.showEvaluation = function () {
    ...
    var scoreText = H5P.Question.determineOverallFeedback(
  ...
  Blanks.prototype.getTitle = function () {
    return H5P.createTitle(
      this.contentData.metadata && this.contentData.metadata.title
...
H5P.Blanks.parseText = function (question) {
  ...
  return tokenizeQuestionText(replaceHtmlTags(question, "")).map(
    function (part) {
