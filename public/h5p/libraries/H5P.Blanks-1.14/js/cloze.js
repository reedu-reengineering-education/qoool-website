  ...
  Blanks.Cloze = function (solution, behaviour, defaultUserAnswer, l10n) {
    ...
    this.setInput = function (
      ...
      inputLabel = inputLabel
        .replace("@num", clozeIndex + 1)
        .replace("@total", totalCloze)

      // Add tip if tip is set
      if (tip !== undefined && tip.trim().length > 0) {
        $wrapper.addClass("has-tip").append(
    ...
    this.toString = function () {
      ...
      var result =
        '<span class="h5p-input-wrapper"><input type="text" class="h5p-text-input" autocomplete="off" autocapitalize="off" spellcheck="false"' +
        extra +
        "></span>"
